...
```{r}
library("vcfR")
leaf_505 <- read.vcfR("~/505/vcf_leaf_no_pop/combined/505_filtered_GQ.vcf.gz")

# save(leaf_505, file="~/505/output/leaf_505.Rdata")
load("~/505/output/leaf_505.Rdata")
dim(leaf_505) # 476781        8       95  
...
#   temp2 <- data.frame(sapply(temp2, function(x) sub("1/1","1",x)))
  temp2 <- data.frame(sapply(temp2, function(x) sub("/","",x)))
#   row.names(temp2) <- rnames
  
#   temp3 <- data.frame(cbind(vcfchrom,vcfpos,vcfref,vcfalt,vcfgqs))
#   colnames(temp3)[1:4] <- c("CHROM","POS","REF","ALT")

#  save(temp2, file="~/505/output/temp2.Rdata")
#  save(temp3, file="~/505/output/temp3.Rdata")

  load("~/505/output/temp2.Rdata") 
#  load("~/505/output/temp3.Rdata")
...
dim(temp2.t) # 98 476781
head(temp2.t)
temp2.t.reform <- temp2.t[5:98,] 
temp2.t.reform[1:8, 1:20]
...
setwd("~/505/output/myGAPIT/")  # ***** need to get this right 

# get genotype data 
geno_505_hmp <- read.table("~/Desktop/Brassica_project/KIAT_RNA_seq/505/data/505_filtered_GQ_sorted.hmp.txt", head=T)
geno_505_hmp <- read.table("~/505/vcf_leaf_no_pop/combined/505_filtered_GQ_sorted.hmp.txt", head=FALSE)
...
myG.03 <- myG[myG$V3==1,]
class(myG.03$V3)
myG.03.final <- data.frame(rbind(myG[1,], myG.03))
myG.03.final[1:10, 1:10] 
dim(myG.03.final) # 21415 105
myG.03.final$V3[2:21415] <- as.numeric(as.character(myG.03.final$V3[2:21415]))
class(myG.03.final$V3[2]) 

dim(myY)
dim(myG.03.final)
### maybe I need the same order for genotype & phenotype data??? 
# get the order of genotype for phenotype 
