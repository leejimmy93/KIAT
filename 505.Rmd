...
temp <- leaf_505 
  ...
  vcfgls <- subset(extract.gt(temp, element = "GL", IDtoRowNames = F), subset = vcfbi) # get gl 

####### filter for GL 
# for(i in 1:length(vcfgls)){
#   if(!is.na(vcfgls[i])){
#     if(sum(sapply(as.numeric(unlist(strsplit(vcfgls[i],","))), function(x) (x > -2 && x != 0))) >= 1){
#       vcfgls[i] <- NA
#     }
#   }
# }
...
p.mds.country <- p.mds.country + geom_point(size=1) + geom_text(size=2)
...
ggsave(p.mds.country, filename="~/505/output/figure/p.mds.country.png", height=10, width=10)
