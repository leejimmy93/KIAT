...
# check the genes reside nearby the significant SNPs 
```{r}
library(IRanges)
...
gffRangeData <- import.gff("~/Reference/B.napus/Brassica_napus.annotation_v5.sorted.gff3") 
...
gff.mRNA.df <- as.data.frame(disjoin(gff.mRNA)) # after disjoin, output file is sorted based on POS 
...
Erucic_acid <- read.csv("~/505/output/myGAPIT/leaf/missing_rate_0.5_A01/GAPIT..Erucic_acid.GWAS.Results.csv", header = T)

Oleic_acid <- read.csv("~/505/output/myGAPIT/leaf/missing_rate_0.5_A01/GAPIT..Oleic_acid.GWAS.Results.csv", header = T)

head(Erucic_acid)
Erucic_acid[Erucic_acid$P.value < 0.00001,]$SNP
...
SNP.Erucic$CHROM <- as.character(SNP.Erucic$CHROM)
gff.mRNA.df$seqnames <- as.character(gff.mRNA.df$seqnames)
head(SNP.Erucic)
head(gff.mRNA.df)

gff.mRNA.df.chrA08 <- gff.mRNA.df[gff.mRNA.df$seqnames=="chrA08",] 
head(gff.mRNA.df.chrA08)
tail(gff.mRNA.df.chrA08)

for (i in 1:nrow(SNP.Erucic)){
  for (j in 1:nrow(gff.mRNA.df)){
  if (
      (SNP.Erucic$CHROM[i]==gff.mRNA.df$seqnames[j]) &  
      (SNP.Erucic$POS[i] <= gff.mRNA.df$end[j]) & 
      (SNP.Erucic$POS[i] >= gff.mRNA.df$start[j])
      ){
    SNP.Erucic$Name[i] <- gff.mRNA.df$Name[j]
    }
  }
}

# change the above code to apply 
