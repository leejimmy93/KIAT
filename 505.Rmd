...
## pull out the TMM normalized expression value for the 20 unique genes 
SNP.gene.fxn <-read.csv("~/505/output/leaf/GWAS_SNP_gene_fxn.csv", header=T)
SNP.gene.fxn 
# how to look at the result? categorize trait into categorical value? and compare between categories?
tmp <- as.data.frame(dge.read.count$counts[which(rownames(dge.read.count$counts) %in% SNP.gene.fxn$gene_ID),])
tmp2 <- read.count.small[which(rownames(read.count.small) %in% SNP.gene.fxn$gene_ID),]
dim(tmp2)
length(unique(SNP.gene.fxn$gene_ID)) 
...
>>>>>>> 3cb176ca171dbf9c518ad9f860c62f4f7d2f6dc2
```
