...
```{r}
library("vcfR")
temp <- read.vcfR("505_filtered_MAF_missingrate_Q_depth.gz")
  vcfbi <- is.biallelic(temp)
  vcfref <- subset(getREF(temp), subset = vcfbi)
  vcfalt <- subset(getALT(temp), subset = vcfbi)
  vcfgts <- subset(extract.gt(temp, element = "GT"), subset = vcfbi)
  vcfpos <- subset(getPOS(temp), subset = vcfbi)
  vcfchrom <- subset(getCHROM(temp), subset = vcfbi)
...
head(temp2)
temp2.5000 <- temp2[1:5000,] 

geno.numeric <- data.matrix(temp2.5000)
