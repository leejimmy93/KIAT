...
# 4) extract only SNPs 
# extract_SNP.sh input.vcf output 
...
save(vcf.freebayes.HQ.2.test1.Ae, file = "~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/freebayes/vcf.freebayes.HQ.2.test1.Ae.Rdata") 
...
vcf.freebayes.HQ.2.test1.Ae.het$alt_ratio <- round(vcf.freebayes.HQ.2.test1.Ae.het$Ae_alt.depth/(vcf.freebayes.HQ.2.test1.Ae.het$Ae_ref.depth+vcf.freebayes.HQ.2.test1.Ae.het$Ae_alt.depth), digits = 2)

hist(log10(vcf.freebayes.HQ.2.test1.Ae.het$alt_ratio), breaks= 150) 


nrow(vcf.freebayes.HQ.2.test1.Ae.het[vcf.freebayes.HQ.2.test1.Ae.het$alt_ratio>0,])
nrow(vcf.freebayes.HQ.2.test1.Ae.het) 
min(vcf.freebayes.HQ.2.test1.Ae.het$alt_ratio) # 0.2 
max(vcf.freebayes.HQ.2.test1.Ae.het$alt_ratio) # 0.92

pl <- ggplot(data = vcf.freebayes.HQ.2.test1.Ae.het) 
pl <- pl + geom_histogram(aes(alt_ratio)) 
...
# format SNP primer for oligo synthesis 
```{r}
primer <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/primer_SNP_validation.csv", header = T)

# seq_ID is SNP ID
loc <- expand.grid(c("A", "B", "C", "D", "E", "F", "G", "H"), 1:12)
loc.plate <- paste(loc$Var1, loc$Var2, sep = "")
loc.plate

primer.F <- data.frame(Oligo_name = paste(primer$sample_ID, "F", sep = ""), 
                           Oligo_seq = primer$primer.F,
                           location = loc.plate,
                           scale = "25nm"
                      )

primer.R <- data.frame(Oligo_name = paste(primer$sample_ID, "R", sep = ""), 
                           Oligo_seq = primer$Primer.R,
                           location = loc.plate,
                           scale = "25nm"
                       )
...
write.csv(primer.F, "~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/primer_F.csv")
write.csv(primer.R, "~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/primer_R.csv")

# checking PCR product 
primer$location <- loc.plate
order <- expand.grid(1:12, c("A", "B", "C", "D", "E", "F", "G", "H"))
order.well <- paste(order$Var2, order$Var1, sep = "")

primer$location <- factor(primer$location, levels = order.well)
primer <- primer[order(primer$location),]
colnames(primer)
matrix(primer[,c("ampmlicon.size")], 4, 24)
```

Use Uprendra's data to try my pipeline 
```{r}
# 1) select data for STAR mapping, all I have is single end data 
# dir=/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/2016_fall/BIS180L/trimming_mapping/split_fq 
# data: IMB211_NDP_1_PETIOLE.fq; IMB211_NDP_2_PETIOLE.fq; IMB211_NDP_3_PETIOLE.fq; R500_NDP_1_PETIOLE.fq; R500_NDP_2_PETIOLE.fq; R500_NDP_3_PETIOLE.fq 

# 2) build star genome of B.rapa 
# download B.rapa v2.1 (PacBio data) tried to build star genome, but was limited by memory 
# there is already a star genome built, maybe that one was based on V1.5??? 

# combined fq data --> mapping use STAR 
# 

# prep4freebayes --> sort bam file --> SNP calling (final bam file should be sorted again, otherwise error !!!)

######## analyze vcf result 
# import data 
R500_IMB211 <- read.table("~/SNP_parent/Upendra_data/R500_IMB211_filtered.vcf", as.is=T, na.strings=".")
dim(R500_IMB211)
head(R500_IMB211)

# get header 
header <- system("grep '#C' ~/SNP_parent/Upendra_data/R500_IMB211.vcf",intern = TRUE) 
header <- sub("#","",header) #get rid of the pound sign
...
SNP.freebayes.reformat <- function(vcf, vcf.header){ 
  ...
  vcf$R500[is.na(vcf$R500)] <- "NA:NA:NA:NA:NA:NA:NA:NA"
  vcf$IMB211[is.na(vcf$IMB211)] <- "NA:NA:NA:NA:NA:NA:NA:NA"
  
  R500.tmp.unique <- matrix(
    unlist(strsplit(vcf$R500,split = ":")),
    nrow=nrow(vcf),  
    byrow=TRUE
  )

  IMB211.tmp.unique <- matrix(
    unlist(strsplit(vcf$IMB211,split = ":")),
    nrow=nrow(vcf),  
    byrow=TRUE
  ) 
  
  colnames(R500.tmp.unique) <- paste("R500",c("gt", "gt.qual", "tot.depth","ref.depth","ref.qual","alt.depth","alt.qual","gen.lik"),sep="_")
  colnames(IMB211.tmp.unique) <- paste("IMB211",c("gt", "gt.qual", "tot.depth","ref.depth","ref.qual","alt.depth","alt.qual","gen.lik"),sep="_")
  
  vcf.reform <- cbind(vcf,R500.tmp.unique,IMB211.tmp.unique,stringsAsFactors=FALSE)
  
  vcf.reform[,c("R500_tot.depth","R500_gt.qual","R500_ref.depth","R500_ref.qual","R500_alt.depth","R500_alt.qual")] <- 
    apply(vcf.reform[,c("R500_tot.depth","R500_gt.qual","R500_ref.depth","R500_ref.qual","R500_alt.depth","R500_alt.qual")],
          2,
          as.numeric) 
  
  vcf.reform[,c("IMB211_tot.depth","IMB211_gt.qual","IMB211_ref.depth","IMB211_ref.qual","IMB211_alt.depth","IMB211_alt.qual")] <- 
    apply(vcf.reform[,c("IMB211_tot.depth","IMB211_gt.qual","IMB211_ref.depth","IMB211_ref.qual","IMB211_alt.depth","IMB211_alt.qual")],
          2,
          as.numeric) 
...
#########
R500_IMB211.reform <- SNP.freebayes.reformat(vcf = R500_IMB211, vcf.header = header)
dim(R500_IMB211.reform) # 145556     18    
table(R500_IMB211.reform$R500_gt) 
table(R500_IMB211.reform$IMB211_gt)     

######## 
# MAF distribution for heterzygous call 
R500_IMB211.reform$MAF_R500 <- round(pmin(R500_IMB211.reform$R500_alt.depth,  R500_IMB211.reform$R500_ref.depth)/R500_IMB211.reform$R500_tot.depth, digits = 2) 

R500_IMB211.reform$R500_allele_ratio <- round(R500_IMB211.reform$R500_alt.depth/R500_IMB211.reform$R500_ref.depth, digits = 2)

hist(R500_IMB211.reform$MAF_R500)
# hist(R500_IMB211.reform[R500_IMB211.reform$R500_gt=="0/1",]$R500_allele_ratio)
hist(R500_IMB211.reform[R500_IMB211.reform$R500_gt=="0/1",]$MAF_R500)
``` 
...
pl.SNP.main.2.unique <- pl.SNP.main.2.unique + geom_histogram(aes(x=POS, fill=subgenome), binwidth = 1000000) 
pl.SNP.main.2.unique <- pl.SNP.main.2.unique + facet_grid(subgenome ~ Chr_ID)
pl.SNP.main.2.unique <- pl.SNP.main.2.unique + labs(list(title = "", x = "physical position", y = "number of SNPs"))
pl.SNP.main.2.unique <- pl.SNP.main.2.unique + theme(legend.position = "none", panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_fill_manual(values=c("red", "blue"))
pl.SNP.main.2.unique  

# overlay with gene density 
...
gff.mRNA.main %>% 
  ...
  facet_grid(subgenome ~ chrom) + 
  theme(legend.position = "none", panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_fill_manual(values=c("red", "blue")) + 
...
# circos plot combing SNP and gene density 
gene_density <- 
gff.mRNA.main %>% 
  ggplot() + 
  layout_circle(aes(x=start, fill=subgenome), binwidth = 1000000) + 
  facet_grid(subgenome ~ chrom) + 
  theme(legend.position = "none", panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_fill_manual(values=c("red", "blue")) + 
  labs(x = "gene start position", y = "number of genes")  
