...
parents_stats_final <- 
parents_stats %>% 
  mutate(genotype = Cultivar, tissue = Stage) %>% 
  dplyr::select(SampleID, genotype, tissue, TotalReads, Q20_MoreBasesRate, Percent_Unique_Mapped, Percent_Multi_Mapped, Percent_Unmapped_Too_Short, Percent_Unmapped_Other) 

parents_stats_final$Q20_MoreBasesRate <- as.character(parents_stats_final$Q20_MoreBasesRate)
parents_stats_final$Q20_MoreBasesRate[9:27] <- as.numeric(gsub("%", "", parents_stats_final$Q20_MoreBasesRate[9:27]))
parents_stats_final$Q20_MoreBasesRate[1:8] <- as.numeric(parents_stats_final$Q20_MoreBasesRate[1:8]) * 100
parents_stats_final %>% View() 

write.csv(parents_stats_final, file = "~/Desktop/F2_paper/supplementary/parents_stats_final.csv")

# F2 
mapping_result_F2_AB <- read.table("~/Desktop/F2_paper/data/Star_Stats_AB.tab", header = T) 
mapping_result_F2_AB$Sample <- gsub("138-", "F2_", mapping_result_F2_AB$Sample)   
mapping_result_F2_AB$Sample <- gsub(".paired", "", mapping_result_F2_AB$Sample)   

mapping_result_F2_C <- read.table("~/Desktop/F2_paper/data/Star_Stats_C.tab", header = T) 
mapping_result_F2_C$Sample <- gsub("Sample_", "Sample_F2_", mapping_result_F2_C$Sample)   

colnames(mapping_result_F2_AB)
mapping_result_F2 <- 
rbind(mapping_result_F2_AB, mapping_result_F2_C) %>% 
...
write.csv(mapping_result_F2, file = "~/Desktop/F2_paper/supplementary/mapping_result_F2.csv")
...
p.average.HQ <- p.average.HQ + theme(axis.text.x = element_text(angle = 90), legend.position = "none")
p.average.HQ <- p.average.HQ + geom_text(data = average.mapping.long.HQ, aes(x=Group.1, y=value, label=factor(round(value,0)))) 
p.average.HQ

ggsave(p.average.HQ, filename =  "~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/output/figure/average_HQ.png", height = 4, width = 2) 
...
p.average.mapping <- p.average.mapping + geom_bar(aes(x=variable, y=value, fill=variable), stat = "identity")
p.average.mapping <- p.average.mapping + facet_wrap(~Group.1, ncol = 2)
p.average.mapping <- p.average.mapping + labs(list(title = "mapping stats of Da-Ae & Da-Ol-1", x = "", y = "percentage of HQ reads"))
p.average.mapping <- p.average.mapping + theme(axis.text.x = element_text(angle = 90), legend.position = "none")
...
ggsave(p.average.mapping, filename =  "~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/output/figure/average_mapping.png", height = 6, width = 5) 
```
...
# PASA annotation..., final result is what? updated cds.fa? 
...
"BnaA08g12780D" %in% rownames(DEgene.new.gt) # FAD 
"BnaC03g67820D" %in% rownames(DEgene.new.gt) # FAD
"BnaA08g11140D" %in% rownames(DEgene.new.gt) # 
"BnaA08g11060D" %in% rownames(DEgene.new.gt) # squalene synthase 
"BnaA08g11130D" %in% rownames(DEgene.new.gt) # FAE
"BnaC03g65980D" %in% rownames(DEgene.new.gt) # FAE
...
save(DEgene.new.gt, DEgene.new.tissue, DEgene.new.interaction, file = "~/Desktop/Brassica_project/KIAT_RNA_seq/output/parent/DEgene.new.Rdata") 
...
pl.heatmap1 <- pl.heatmap1 + geom_tile(color = "black", aes(x = factor(variable), y = Term, fill=logPvalue)) + scale_fill_gradient2(low=muted("green"), high=("royalblue")) 
pl.heatmap1 <- pl.heatmap1 + labs(y = "GO term", x="", title=" ") 
pl.heatmap1 <- pl.heatmap1 + theme_classic()
pl.heatmap1 <- pl.heatmap1 + theme(text = element_text(size=7), axis.text=element_text(size=rel(0.8))) 
pl.heatmap1  

ggsave(pl.heatmap1, filename = "~/Desktop/F2_paper/pl.heatmap1.png", width = 5, height = 3)  
# young 
...
dim(napus_vs_ara.final) # 18677    reciprocal blast result 
...
write.table(napus_vs_ara.non_reciprocal, file = "~/Desktop/Brassica_project/reference/napus_vs_ara.non_reciprocal.table")
rownames(napus_vs_ara.non_reciprocal) <- napus_vs_ara.non_reciprocal$napus_ID
head(napus_vs_ara.non_reciprocal)
dim(napus_vs_ara.non_reciprocal) # 64949 non reciprocal blast result 
```
...
# unused 
```{r}
# fitted expression value correlation 
head(fitted.all.3$fitted.values)

fitted.value.all <- data.frame(row.names = row.names(fitted.all.3),
                                 value = as.numeric(rowMeans(fitted.all.3$fitted.values)))

fitted.value.two.rep <- data.frame(row.names = row.names(fitted.all.two.rep),
                                 value = as.numeric(rowMeans(fitted.all.two.rep$fitted.values)))

fitted.value.no.rep <- data.frame(row.names = row.names(fitted.all.no.rep),
                                 value = as.numeric(rowMeans(fitted.all.no.rep$fitted.values)))

fitted.value.three.quarter <- data.frame(row.names = row.names(fitted.three.quarter.3),
                                 value = as.numeric(rowMeans(fitted.three.quarter.3$fitted.values))) 

fitted.value.half <- data.frame(row.names = row.names(fitted.half.3),
                                 value = as.numeric(rowMeans(fitted.half.3$fitted.values)))

fitted.value.one.quarter <- data.frame(row.names = row.names(fitted.quarter.3),
                                 value = as.numeric(rowMeans(fitted.quarter.3$fitted.values)))

ref.vs.rep2 <- as.numeric(cor(fitted.value.all$value, fitted.value.two.rep$value))
ref.vs.rep1 <- as.numeric(cor(fitted.value.all$value, fitted.value.no.rep$value))
ref.vs.three.quarter <- as.numeric(cor(fitted.value.all$value, fitted.value.three.quarter$value))
ref.vs.one.quarter <- as.numeric(cor(fitted.value.all$value, fitted.value.one.quarter$value))
ref.vs.half <- as.numeric(cor(fitted.value.all$value, fitted.value.half$value))

fitted.value.fit <- data.frame(ref.vs.three.quarter, ref.vs.rep2, ref.vs.half, ref.vs.rep1, ref.vs.one.quarter) 
fitted.value.fit.long <- melt(fitted.value.fit)
fitted.value.fit.long

p.fitted.value <- ggplot(data = fitted.value.fit.long)
p.fitted.value <- p.fitted.value + geom_count(mapping = aes(x = variable, y = value), stat = "identity")
p.fitted.value <- p.fitted.value + theme(axis.text.x=element_text(angle=90),strip.text.y = element_text(angle=0)) 
p.fitted.value <- p.fitted.value + labs(y = "fitted value correlation", x = "")

p.fitted.value 
ggsave(p.AveLogCPM, filename = "~/Desktop/Brassica_project/KIAT_RNA_seq/output/p.AveLogCPM.png", width = 4, height = 4)  

dim(fitted.all.3$coefficients)
head(fitted.all.3$coefficients, 1000)
dim(fitted.all.3$fitted.values)
length(fitted.all.3$AveLogCPM)

# fitted AveLogCPM
ref.vs.rep2 <- as.numeric(cor(fitted.all.3$AveLogCPM, fitted.all.two.rep$AveLogCPM))
ref.vs.rep1 <- as.numeric(cor(fitted.all.3$AveLogCPM, fitted.all.no.rep$AveLogCPM))
ref.vs.three.quarter <- as.numeric(cor(fitted.all.3$AveLogCPM, fitted.three.quarter.3$AveLogCPM))
ref.vs.one.quarter <- as.numeric(cor(fitted.all.3$AveLogCPM, fitted.quarter.3$AveLogCPM))
ref.vs.half <- as.numeric(cor(fitted.all.3$AveLogCPM, fitted.half.3$AveLogCPM)) 

AveLogCPM.fit <- data.frame(ref.vs.three.quarter, ref.vs.rep2, ref.vs.half, ref.vs.rep1, ref.vs.one.quarter) 
AveLogCPM.fit.long <- melt(AveLogCPM.fit)
AveLogCPM.fit.long

p.AveLogCPM <- ggplot(data = AveLogCPM.fit.long)
p.AveLogCPM <- p.AveLogCPM + geom_count(mapping = aes(x = variable, y = value), stat = "identity")
p.AveLogCPM <- p.AveLogCPM + theme(axis.text.x=element_text(angle=90),strip.text.y = element_text(angle=0)) 
p.AveLogCPM <- p.AveLogCPM + labs(y = "AveLogCPM correlation", x = "")
...
tmp 
```{r}
tmp <- sample_des %>% select(SampleID, TotalReads) %>% mutate(TotalRead = TotalReads/2) %>% select(SampleID, TotalRead) 
tmp2 <- read.delim("~/Desktop/Brassica_project/KIAT_RNA_seq/tmp", header = F)
tmp2 

tmp %>% head()
tmp2 %>% head()
tmp2$SampleID <- gsub("([[:print:]]+)(_)(1|2)(.fq.gz)", "\\1", tmp2$V1)

test <- left_join(tmp2, tmp, by="SampleID")
test$bam_folder <- paste(test$SampleID, ".star.trim.dir", sep = "")
test$bam_folder_2 <- paste("whitney:/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/2016_summer/raw_data", test$bam_folder, sep = "/")
write.csv(test, file = "~/Desktop/test.csv")

tmp3 <- read.csv("~/Desktop/test.csv")
tmp3$folder2 <- paste("Cabernet:/share/malooflab/Ruijuan/parent_assembly/cufflinks", tmp3$folder, "cufflink_output", sep = "/")
tmp3$folder2
write.csv(tmp3, file = "~/Desktop/test.csv")

load("~/Desktop/Brassica_project/KIAT_RNA_seq/output/parent/DEgene.pairwise.gt.Rdata")
load("~/Desktop/F2_paper/data/cis_eQTL_linolenic.Rdata")

DEgene.young[rownames(DEgene.young) %in% cis_eQTL_linolenic$names,]
DEgene.bolting[rownames(DEgene.bolting) %in% cis_eQTL_linolenic$names,]
DEgene.flowering[rownames(DEgene.flowering) %in% cis_eQTL_linolenic$names,]
DEgene.early.silique[rownames(DEgene.early.silique) %in% cis_eQTL_linolenic$names,]
DEgene.late.silique[rownames(DEgene.late.silique) %in% cis_eQTL_linolenic$names,] 

library(reshape2)
load("/Users/ruijuanli/Desktop/Brassica_project/KIAT_RNA_seq/data/vstMat.parent.Rdata")

expression.pattern.Bn.parent(cis_eQTL_linolenic$names)
cis_eQTL_linolenic$names

cis_eQTL_linolenic <- as.data.frame(cis_eQTL_linolenic$names)
colnames(cis_eQTL_linolenic) <- "V1"
rownames(cis_eQTL_linolenic) <- cis_eQTL_linolenic$V1 

data.melt.reshape %>% filter(tissue == "late-silique")  %>% 
  ggplot() + 
  geom_bar(aes(x = gt, y = mean), stat = "identity") + 
  facet_wrap(~geneID) + 
  geom_errorbar(mapping=aes(x=gt,ymin=min,ymax=max, width=0.25)) 
```
