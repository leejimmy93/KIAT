...
summary(scantwo.imp.42traits[["Erucic_acid"]],perm=scantwo_perm_Erucic,alpha=c(.05, .05, .05, .05, .05),pval=T) 
...
qtl.add.Erucic <- addint(LG.f2.after.crossover,qtl=qtl.refine.Erucic) # no evidence for interactions
summary(qtl.add.Erucic)

qtl.add.Erucic <- addqtl(LG.f2.after.crossover,qtl=qtl.refine.Erucic)
summary(qtl.add.Erucic)

qtlm2.Erucic <- addtoqtl(LG.f2.after.crossover,qtl.refine.Erucic,chr=c("A08"),
                         pos=c(91.0))

summary(qtlm2.Erucic)

#refine the positions of our new model
qtl.refine2.Erucic <- refineqtl(LG.f2.after.crossover,qtl=qtlm2.Erucic,
                         formula = y ~ Q1 + Q2 + Q3)

summary(qtl.refine2.Erucic)
qtl.fit2.Erucic <- fitqtl(LG.f2.after.crossover,qtl=qtl.refine2.Erucic,get.ests=T)
...
#are all of the QTL well supported?  If not, make simpler model using the dropfromqtl() function 
...
# test on other traits
```{r}
summary(scantwo.imp.42traits[["days_to_bolt"]], thresholds = c(2.3, 5.45, 6.95, 4.37, 2.98)) # signal for interaction of the two spots
qtlm.Erucic <- makeqtl(LG.f2.after.crossover,chr=c(10, 16),pos=c(176, 83))
qtlm.Erucic # n.gen is "number of possible genotypes for each chromosome", if F2, n.gen = 3 when doing sim.geno().
qtl.fit.Erucic <- fitqtl(cross=LG.f2.after.crossover,qtl=qtlm.Erucic,formula = y ~ Q1, get.ests=T)
summary(qtl.fit.Erucic) # not supported...???   
```

# marker effect 
```{r}
LG.f2.after.crossover$pheno[,1] <- LG.f2.after.crossover$pheno$Erucic_acid
LG.f2.after.crossover$pheno

LG.f2.after.crossover <- calc.genoprob(LG.f2.after.crossover, step=1) 

qtl <- makeqtl(LG.f2.after.crossover, chr=8, pos=72.3, what="prob") 
out.fitqtl <- fitqtl(LG.f2.after.crossover, qtl=qtl, method="hk") 
summary(out.fitqtl)  
...
qtl <- makeqtl(fake.f2, chr=13, pos=28, what="prob") 
out.fitqtl <- fitqtl(fake.f2, qtl=qtl, method="hk") 
summary(out.fitqtl) 
