...
dim(cis_trans_result_3) # 728717     13 
...
seq_along(trans_candidate_list[[subset]])
...
lapply(seq_along(trans_candidate_list), function(subset) { 
  ...
  lapply(seq_along(trans_candidate_list[[subset]]$gene_ID), function(index) {
    ...
    tmp1[1,c("gene_ID", "LG", "loci_start_2", "loci_end_2")] # the last item is the one that was returned
    })
}
)
) # 52 sec   
...
sum(test4$loci_start < test4$loci_end) # 23770 
sum(test4$loci_start == test4$loci_end) # 44
...
trans_candidate_2 %>% 
  left_join(test4) %>%
  mutate(loci_start = loci_start_2, loci_end = loci_end_2) %>% 
  dplyr::select(-(loci_start_2:loci_end_2))

trans_candidate_3 %>% head()
dim(trans_candidate_3) # 640310     13
...
dim(cis_trans_result_final) # 728717     13 
