...
cis_candidate_3$gene_start %>% class()
cis_candidate_3$loci_start %>% class() 

# use loci and gene start/end position to find cis-eQTL 
cis_candidate_4 <- 
cis_candidate_3 %>% 
  filter((gene_start < loci_start && gene_end > loci_start) || 
         (gene_start > loci_start && gene_end < loci_end) || 
         (gene_start < loci_end && gene_end > loci_end)) 

dim(cis_candidate_4)    

# get the final set of cis- and trans- eQTL, no change after refning, so just keep the original data, but change cis-candidate to cis
