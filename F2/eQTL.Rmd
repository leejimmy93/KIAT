...
save(cis_trans_result_final, file = "~/F2/output/eQTL/cis_trans_result_2.Rdata")
...
lapply(names(eQTL_result[1:10]), function(subset) {
  ...
  lapply(names(eQTL_result[[subset]])[names(eQTL_result[[subset]]) %in% cis_candidate], function(gene) {
    ...
    tmp1$loci_start <- min(tmp1$pos_2)
    tmp1$loci_end <- max(tmp1$pos_2)
...
sum(cis_candidate_3$loci_start < cis_candidate_3$loci_end) # 492708
