...
cis_eQTL %>% dim() #  11710    11  
trans_eQTL %>% dim() # 14871    11  
...
cis_eQTL.qtl.combined %>% dim() # 2200   21 
...
test1 <- read.csv("~/F2/output/eQTL/cis_eQTL.qtl.combined.final.csv")
dim(test1)

test2 <- read.csv("~/F2/output/eQTL/trans_eQTL.qtl.combined.final.csv")
dim(test2)

intersect(test2$gene_ID, trans_eQTL.qtl.combined.final$gene_ID) %>% unique() %>% length()
