...
## also for data with different LOD and rf 
...
# ripple_maximum_likelihood.R (takes extremely long time) --> decided to run for each LG seperately... 
...
LG.f2.madmapper.before.crossover <- switch.order(LG.f2.madmapper.before.crossover, chr = 19, rip2.19[2,])

# after ripple with maximum likelihood, check the result and select orders which gives me the largest LOD and shortest chrlen --> remove double crossver --> ...  

# reestimate map 

############ need to reestimate map after switch order for LG9 
...
<<<<<<< HEAD
plotMap(LG.f2.madmapper.after.crossover)
save(LG.f2.madmapper.after.crossover, file = "~/F2/output/missing_rate_0.10/LG.f2.madmapper.after.crossover.Rdata")
```

### plot new rf plot 
```{r}
load("~/F2/output/missing_rate_0.10/LG.f2.madmapper.after.crossover.Rdata")
plot.rf(LG.f2.madmapper.after.crossover) 
summaryMap(LG.f2.madmapper.after.crossover) # 2878.2  cM 
=======
...
png("~/Desktop/Brassica_project/KIAT_RNA_seq/F2/output/figure /rf_chr10.png", width=10, height=10, units="in", res=300)
plot.rf(LG.f2.madmapper.after.crossover, chr = 'A10', main = "RF and LOD scores for chrom A10")
dev.off() 
>>>>>>> 9eedfc26888ae4f71595541897f5a107228f764e
```
...
summaryMap(LG.f2.madmapper.after.crossover) # 2853 cM 
