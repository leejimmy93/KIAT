...
dim(F2_geno_data_with_missing_rate_filtered) # 5797 167
...
dim(F2_geno_data_with_missing_rate_filtered.final) # 5797 166 
rownames(F2_geno_data_with_missing_rate_filtered.final) 
```
...
dim(F2_geno_data_t.numeric.2) # 166 5795  
...
dim(F2_SNP_correlation) # 5795 5795 

# calcualte number of SNPs with correlation of 1
nrow(which(F2_SNP_correlation == 1 & lower.tri(F2_SNP_correlation), arr.ind = T, useNames = T)) # 251 pairs of SNPs have identical genotype data 
...
length(dup.pair[,2]) # 251 
rem <- unique(dup.pair[,2])
length(rem) # 239 markers are removed due to pairwise correlation of 1 
...
dim(F2_geno_data_2) # 5556  166, end up with 2932 SNPs for map construction 
...
dim(F2_geno_data_2_Ae_Ol_new) # 5556 166  
...
sum(F2_geno_data_2$Chr=="chrA01") # 346 1-346  
sum(F2_geno_data_2$Chr=="chrA02") # 163 347-509
sum(F2_geno_data_2$Chr=="chrA03") # 506 510-1015
sum(F2_geno_data_2$Chr=="chrA04") # 275 1016-1290
sum(F2_geno_data_2$Chr=="chrA05") # 263 1291-1553
sum(F2_geno_data_2$Chr=="chrA06") # 710 1554-2263
sum(F2_geno_data_2$Chr=="chrA07") # 482 2264-2745
sum(F2_geno_data_2$Chr=="chrA08") # 274 2746-3019
sum(F2_geno_data_2$Chr=="chrA09") # 545 3020-3564
sum(F2_geno_data_2$Chr=="chrA10") # 474 3565-4038
sum(F2_geno_data_2$Chr=="chrC01") # 196 4039-4234
sum(F2_geno_data_2$Chr=="chrC02") # 112 4235-4346
sum(F2_geno_data_2$Chr=="chrC03") # 322 4347-4668
sum(F2_geno_data_2$Chr=="chrC04") # 245 4669-4913
sum(F2_geno_data_2$Chr=="chrC05") # 82 4914-4995
sum(F2_geno_data_2$Chr=="chrC06") # 161 4996-5156
sum(F2_geno_data_2$Chr=="chrC07") # 192 5157-5348
sum(F2_geno_data_2$Chr=="chrC08") # 131 5349-5479
sum(F2_geno_data_2$Chr=="chrC09") # 77 5480-5556  
...
load("~/Desktop/Brassica_project/KIAT_RNA_seq/F2/output/missing_rate_0.15/right/twopts.f2.05.12.Rdata") # LOD=3, max rf=0.5 
...
# after practicing madmapper with eg data, now I am running it on my data 
```

[http://cgpdb.ucdavis.edu/XLinkage/MadMapper/Genetic_Map_MadMapper_Arabidopsis.html]

# running madmapper 
# 1) format marker data to madmapper accepted format 
```{r}
# cat F2_geno_data.txt | sed 's/"//g' | tr " " "\t" > F2_geno_data_m.txt 
# vim edit add ";" to the 1st line 
# seq 166 | tr "\n" "\t" > number_line_header 
# vim edit add ";" to number_line_header
# cat number_line_header F2_geno_data_m.txt
...
# the output file we should check is tree.cluster file 
```

# get linkage group from madmapper generated tree.cluster file 
```{r}
