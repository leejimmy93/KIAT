...
```{r}
plot.rf(LG.f2.madmapper.after.crossover, chr = '19')
LG.f2.madmapper.after.crossover$geno$`1`$data
LG.f2.mappingpos.before.crossover$geno$`1`
plot.rf(LG.f2.mappingpos.before.crossover, chr = '10')
```

### continue refine map (refer to Cody's code)
```{r}
...
# write.csv(pull.geno(LG.f2.mappingpos.before.crossover),file="~/Desktop/Brassica_project/KIAT_RNA_seq/F2/output/LG.f2.mappingpos.before.crossover.csv") 

f2.map.new <- est.map(LG.f2.mappingpos.before.crossover,verbose=T,error.prob=.01) # why going through
