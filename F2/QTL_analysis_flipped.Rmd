...
final_FA %>% 
  ggplot() +
  geom_tile(color = "white", aes(x = factor(pos), y = trait, fill=log10(lod))) +  scale_fill_gradient2(low=("green"), mid = "white", high=("magenta"), midpoint = 0, limits = c(-2, 2)) +
  # geom_tile(color = "white", aes(x = factor(pos), y = trait, fill=lod)) +  scale_fill_gradient2(low=("green"), mid = "white", high=("magenta"), midpoint = 1, breaks = c(1, 2, 3, 4, 40)) + 
...
final_flower_etc %>%  
  filter(trait != "Myristic_acid") %>% 
  ggplot() +
  geom_tile(color = "white", aes(x = factor(pos), y = trait, fill=lod)) + scale_fill_gradient2(low=("green"), high=("magenta"), midpoint = 2.5) +
  facet_grid(trait~chr, scales = "free") + 
...
ggsave(p.flower_etc, filename = "~/F2/for_paper/flower_etc.png", height = 70, width = 174, dpi = 300, units = "mm")
ggsave(p.final_FA, filename = "~/F2/for_paper/FA.png", height = 70, width = 174, dpi = 300, units = "mm")  
```
...
dim(traitQTL.annotated.SNP) # 4062 14
...
# fatty acid genes 
traitQTL.annotated.SNP[grep("lipid|fatty", traitQTL.annotated.SNP$GO_des),] %>%
  dplyr::select(trait, gene_ID, AGI, At_symbol, GO_des, Ae.gt, Ol.gt, mutation_type) %>%
    View() # sequalene synthase 

# transcription factors  
test <- 
traitQTL.annotated.SNP[grep("DNA|nucleotide|transcription", traitQTL.annotated.SNP$GO_des),] %>%
  dplyr::select(trait, gene_ID, AGI, At_symbol, GO_des, Ae.gt, Ol.gt, mutation_type)  # sequalene synthase 

test[grep("acid|aicd|content", test$trait, invert = T),] %>% 
  dplyr::select(trait, gene_ID, AGI, At_symbol, GO_des, Ae.gt, Ol.gt, mutation_type) %>% 
  View()

tmp <- 
test[grep("acid|contents", test$trait),] %>%
  dplyr::select(trait, gene_ID, AGI, At_symbol, GO_des, Ae.gt, Ol.gt, mutation_type) %>% 
  group_by(AGI, gene_ID) %>% 
  summarise(count = n()) %>% 
  filter(count >= 2) %>% 
  dplyr::select(AGI) 

test[test$AGI %in% tmp$AGI,] %>% View()
  # sequalene synthase 

# flowering related genes
colnames(traitQTL.annotated)
traitQTL.annotated %>% 
  filter(AGI == "AT1G69120" | AGI == "AT1G69570") %>% 
  dim() 

# non fatty acid trait 

traitQTL.annotated.SNP[grep("acid|content", traitQTL.annotated.SNP$trait, invert = T),] %>% # enrichment analysis 
  dplyr::select(trait, gene_ID, AGI, At_symbol, GO_des, Ae.gt, Ol.gt, mutation_type) %>% # sequalene synthase 
  group_by(trait) %>% 
  summarise(count = n())

traitQTL.annotated.SNP[grep("acid|content|aicd", traitQTL.annotated.SNP$trait, invert = T),] %>% # enrichment analysis 
  dplyr::select(trait, gene_ID, AGI, At_symbol, GO_des, Ae.gt, Ol.gt, mutation_type) %>% # sequalene synthase 
  View() 
...
traitQTL.annotated.SNP[grep("DNA|nucleotide|transcription", traitQTL.annotated.SNP$GO_des),] %>% 
  filter(chrom == "C06") %>% 
  dplyr::select(gene_ID, trait, AGI, At_symbol, GO_des, Ae.gt, Ol.gt, mutation_type) %>% View()
...
number_cis_coding[grep("acid|content", number_cis_coding$trait),] 

three_genes <- 
traitQTL.annotated.SNP %>% 
  filter(gene_ID == "BnaA10g21380D" | gene_ID == "BnaA10g22000D" | gene_ID == "BnaC06g30620D") 
  
write.csv(three_genes, file = "~/F2/output/three_genes.csv")
```   
