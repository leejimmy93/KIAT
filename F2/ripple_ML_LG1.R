...
#   sfLapply(names(LG.f2.madmapper.before.crossover$geno), function(LG) {
    ripple(cross = LG.f2.madmapper.before.crossover, window = 3, chr = 1, method = "likelihood", error.prob = 0.001)
)
#   })

# sfStop()

save(rip2.1, file = "~/F2/output/missing_rate_0.10/rip2.1.Rdata")
