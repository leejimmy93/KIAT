...
cis_eQTL.qtl.combined %>% 
  dplyr::select(trait.x, gene_ID, trait.y, subject, At_symbol) %>% 
  View()

cis_eQTL.qtl.combined %>% 
  # dplyr::select(trait.x, gene_ID, trait.y, subject, At_symbol) %>% 
  filter(trait.y == "Erucic_acid") %>% 
  dplyr::select(start.y, end.y)
...
# sapply(unique(napus_GO$gene_ID), function(i) 
#   )

test <- 
napus_GO %>% 
  dplyr::filter(gene_ID == "BnaA01g00010D") 
