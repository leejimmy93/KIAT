...
### find homeologous by reprocal blast 
linolenic_acid_test_gene_eQTL <- cis_eQTL.qtl.combined.final.linolenic$gene_ID %>% unique()
write.csv(linolenic_acid_test_gene_eQTL, file = "~/F2/for_paper/linolenic_acid/linolenic_acid_test_gene_eQTL.csv")

# below on linux 
# cat linolenic_acid_test_gene_eQTL.csv | awk 'BEGIN{FS=","}{print $2}' | sed 's/"//g' | grep "BnaA" > A08_test_genes_ID_2
# cat linolenic_acid_test_gene_eQTL.csv | awk 'BEGIN{FS=","}{print $2}' | sed 's/"//g' | grep "BnaC" > C03_test_genes_ID_2

# perl -ne 'if(/^>(\S+)/){$c=$i{$1}}$c?print:chomp;$i{$_}=1 if @ARGV' A08_test_genes_ID_2 /Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/Reference/B.napus/Brassica_napus.annotation_v5.cds.fa > A08_test_genes_2.fa

# perl -ne 'if(/^>(\S+)/){$c=$i{$1}}$c?print:chomp;$i{$_}=1 if @ARGV' C03_test_genes_ID_2 /Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/Reference/B.napus/Brassica_napus.annotation_v5.cds.fa > C03_test_genes_2.fa

# makeblastdb -in A08_test_genes_2.fa -input_type fasta -dbtype nucl -out A08_test_genes_2
# makeblastdb -in C03_test_genes_2.fa -input_type fasta -dbtype nucl -out C03_test_genes_2

# ./reciprocal_blast.sh get reciprocal blast result, no new gene pairs compared to using just AGI blast hit 
...
allele_effect_result ## all possible genes have positive allele effect from Da-Ol-1 

# check the allele effect for all genes? not filtering for homeologous pair, using /Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/KIAT/F2/linolenic_cis_eQTL.R 

load("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/F2/for_paper/linolenic_acid/cis_eQTL_effect.Rdata")
cis_eQTL_effect %>% length()
# next, check result to see whether there are negative effect  
names(cis_eQTL_effect) <- cis_eQTL.qtl.combined.final.linolenic$gene_ID

allele_effect_result <- sapply(names(cis_eQTL_effect), function(gene) cis_eQTL_effect[[gene]]$ests[2,1])
allele_effect_result <- allele_effect_result %>% as.data.frame() ## all possible genes have positive allele effect from Da-Ol-1 
allele_effect_result$names <- rownames(allele_effect_result) 
colnames(allele_effect_result)[1] <- "cis_effect"
allele_effect_result 

# add annotation for these genes, Ara homolog & B.napus GO annotation 
...
colnames(napus_GO_combined)  # dosen't seem to have good candidates 
save(allele_effect_result, file = "~/F2/for_paper/linolenic_acid/allele_effect_result.Rdata")
...
### can also check SNP effect, see whether there are genes fit the homeologous gene model Julin suggested 
```{r}
F2_geno_data <- read.table("~/F2/data/linolenic_acid/F2_Final_SNP_Calls", header = T)
head(F2_geno_data) 
dim(F2_geno_data) # 18226   172  
...
F2_geno_data_interval <- 
F2_geno_data %>% 
  filter((CHROM == "chrA08" & as.numeric(POS) >=  (A08.interval$start - 1000000) & as.numeric(POS) <= (A08.interval$end + 1000000)) | (CHROM == "chrC03" & as.numeric(POS) >=  (C03.interval$start - 1000000) & as.numeric(POS) <= (C03.interval$end + 1000000)))
  
dim(F2_geno_data_interval) # 404 168  
F2_geno_data_interval %>% colnames()

F2_geno_data_interval_linolenic <- 
F2_geno_data_interval %>% 
  dplyr::select(CHROM, POS, REF, ALT, Ae.gt, Ol.gt) 

write.csv(F2_geno_data_interval_linolenic, file = "~/F2/for_paper/linolenic_acid/F2_geno_data_interval_linolenic.csv")

# cat F2_geno_data_interval_linolenic.csv | sed 's/"//g' | sed 's/,/ /g' | awk '{print $2 "_" $3}' | sed 's/chr/S/g' | tail -404 > significant_SNP

# cat Ae_Ol_modified_SNPonly_biallelic.recode.ann.vcf | grep -v "#" | awk '{print $0,  $1 "_" $2}' > Ae_Ol_modified_SNPonly_biallelic.recode.ann.modified.vcf

# grep -f significant_SNP /Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/SNP_parent/result/freebayes/Ae_Ol_modified_SNPonly_biallelic.recode.ann.modified.vcf | awk '{print $1 "|" $2 "|" $4 "|" $5 "|" $8}' | awk 'BEGIN{FS="|"}{print $1, $2, $3, $4, $6}' > significant_SNP_annotation

sig_SNP_ann <- read_table2("~/F2/for_paper/linolenic_acid/significant_SNP_annotation", col_names=F)
head(sig_SNP_ann)  
colnames(sig_SNP_ann) <- c("CHROM", "POS", "REF", "ALT", "mutation_type")
...
SNP <- GRanges(seqnames = Rle(F2_geno_data_interval_linolenic$CHROM), ranges = IRanges(start = F2_geno_data_interval_linolenic$POS, end = F2_geno_data_interval_linolenic$POS), ID = paste(F2_geno_data_interval_linolenic$CHROM, F2_geno_data_interval_linolenic$POS, sep = "_"))
SNP
...
F2_geno_data_interval_linolenic$SNP.ID <- paste(F2_geno_data_interval_linolenic$CHROM, F2_geno_data_interval_linolenic$POS, sep = "_")

SNP_interval_linolenic <- 
F2_geno_data_interval_linolenic %>% 
  left_join(SNP_gene_final) 

write.csv(SNP_interval_linolenic, file = "~/F2/for_paper/linolenic_acid/SNP_interval_linolenic.csv")  

# send this SNP_interval_linolenic file to John, so that John can do provean analysis on these genes to see whether there are deleterious mutation on homeologous genes 

# deleterious_genes.txt is the names of the 22 genes which had a variation that scored -2.0 or less. On Provean's site they consider for human anything less than -2.5 to be a deleterious change

deleterious_genes <- read.table("~/F2/for_paper/linolenic_acid/deleterious_genes.txt")
colnames(deleterious_genes) <- "names"

# add annotation to these genes 
load("~/Reference/B.napus/BnapusAnnotation.Rdata") 
load("~/Reference/B.napus/napus_GO_combined.Rdata")
...
While there was no good candidate from either expression or SNP analysis, I think I might should have used the SNPs between Da-Ae and Da-Ol-1, dosn't consider whether they have good coverage in the F2s.
```{r}
Ae_Ol <- read.csv("~/F2/for_paper/linolenic_acid/vcf.Ae.Ol.intersect.2.csv", header = T, row.names = 1)
head(Ae_Ol) 
dim(Ae_Ol) # 63981     6  
...
dim(F2_geno_data_interval) # 1472 6  
F2_geno_data_interval %>% colnames()
...
write.csv(F2_geno_data_interval_linolenic, file = "~/F2/for_paper/linolenic_acid/F2_geno_data_interval_linolenic_2.csv")

# cat F2_geno_data_interval_linolenic_2.csv | sed 's/"//g' | sed 's/,/ /g' | awk '{print $2 "_" $3}' | sed 's/chr/S/g' | tail -1472 > significant_SNP_2

# cat Ae_Ol_modified_SNPonly_biallelic.recode.ann.vcf | grep -v "#" | awk '{print $0,  $1 "_" $2}' > Ae_Ol_modified_SNPonly_biallelic.recode.ann.modified.vcf

# cat significant_SNP_2 | sed 's/S//g' | awk '{print "chr" $0}' > significant_SNP
...
F2_geno_data_interval_linolenic %>% 
  left_join(SNP_gene_final) 
dim(SNP_interval_linolenic) # 1472  

save(SNP_interval_linolenic, file = "~/F2/for_paper/linolenic_acid/SNP_interval_linolenic.Rdata")
load("~/Reference/B.napus/BnapusAnnotation.Rdata") 
load("~/Reference/B.napus/napus_GO_combined.Rdata")
...
homeo_AGI <- homeo_AGI[!is.na(homeo_AGI)]

SNP_interval_linolenic_annotated <-   
SNP_interval_linolenic %>% 
  left_join(BnapusAnnotation, c("names" = "name")) %>% 
  left_join(napus_GO_combined, c("names" = "gene_ID")) 

candidate_linolenic <- 
SNP_interval_linolenic_annotated[SNP_interval_linolenic_annotated$AGI %in% as.character(unlist(homeo_AGI)),] 

write.csv(candidate_linolenic, file = "~/F2/for_paper/linolenic_acid/candidate_linolenic.csv")
write.csv(SNP_interval_linolenic_annotated, file = "~/F2/for_paper/linolenic_acid/SNP_interval_linolenic_2_annotated.csv")   

SNP_interval_linolenic_annotated[SNP_interval_linolenic_annotated$AGI %in% as.character(unlist(homeo_AGI)),] %>% 
  dplyr::select(CHROM, mutation_type, SNP.ID, names, AGI, At_symbol) %>% 
  View()

SNP_interval_linolenic_annotated %>% 
  filter(names == "BnaA08g11130D") # FAE1

SNP_interval_linolenic_annotated %>% 
  filter(names == "BnaC03g65980D") # FAE1

SNP_interval_linolenic_annotated %>% 
  filter(names == "BnaA08g11060D") # SQS

SNP_interval_linolenic_annotated %>% 
  filter(names == "BnaA08g11140D") # KCS17

SNP_interval_linolenic_annotated %>% 
  filter(names == "BnaA08g12780D") # FAD6 

SNP_interval_linolenic_annotated %>% 
  filter(names == "BnaA08g27750D") # KCS2

SNP_interval_linolenic_annotated %>% 
  filter(names == "BnaA08g12800D") # FAD6 

# John run PROVEAN on the list of SNPs that I sent to him, 
provean.result <- read.table("~/F2/for_paper/linolenic_acid/provean_results.txt")
candidate_linolenic %>% 
  filter(names == "BnaA08g10160D")

provean.result <- read.csv("~/Desktop/F2_paper/data/candidate_linolenic.csv")
provean.result %>% colnames()
provean.result %>% 
  dplyr::select(names, AGI, At_symbol, mutation_type, GO_des:X.4) %>% 
  View()  

# we are worried that using Arabidopsis blast hit to find homeologou might not be the best method, so I will use reciprocal blast to look for homeologs on A08 and C03 for linolenic acid QTL region 

load("~/F2/for_paper/linolenic_acid/SNP_interval_linolenic.Rdata")
linolenic_acid_test_gene <- SNP_interval_linolenic$names %>% unique() 
write.csv(linolenic_acid_test_gene, file = "~/F2/for_paper/linolenic_acid/linolenic_acid_test_gene.csv")

# below on linux 
# cat linolenic_acid_test_gene.csv | awk 'BEGIN{FS=","}{print $2}' | sed 's/"//g' | grep "BnaA" > A08_test_genes_ID
# cat linolenic_acid_test_gene.csv | awk 'BEGIN{FS=","}{print $2}' | sed 's/"//g' | grep "BnaC" > C03_test_genes_ID

# perl -ne 'if(/^>(\S+)/){$c=$i{$1}}$c?print:chomp;$i{$_}=1 if @ARGV' A08_test_genes_ID /Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/Reference/B.napus/Brassica_napus.annotation_v5.cds.fa > A08_test_genes.fa

# perl -ne 'if(/^>(\S+)/){$c=$i{$1}}$c?print:chomp;$i{$_}=1 if @ARGV' C03_test_genes_ID /Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/Reference/B.napus/Brassica_napus.annotation_v5.cds.fa > C03_test_genes.fa

# makeblastdb -in A08_test_genes.fa -input_type fasta -dbtype nucl -out A08_test_genes
# makeblastdb -in C03_test_genes.fa -input_type fasta -dbtype nucl -out C03_test_genes

# ./reciprocal_blast.sh get reciprocal blast result 
blast_result <- read.delim(file = "~/F2/for_paper/linolenic_acid/reciprocal.blast", header = F)
blast_result

homeo_blast <- 
  c(as.character(blast_result$V1), as.character(blast_result$V2))

homeo_blast <- 
  homeo_blast[!(homeo_blast %in% (candidate_linolenic$names %>% unique()))] 

candidate_linolenic_2genes <- 
SNP_interval_linolenic_annotated[SNP_interval_linolenic_annotated$names %in% homeo_blast,] 

candidate_linolenic_2genes %>% 
  dplyr::select(Ae.gt, Ol.gt, names, mutation_type, AGI, At_symbol) %>% 
  View()
  
write.csv(candidate_linolenic_2genes, file = "~/F2/for_paper/linolenic_acid/candidate_linolenic_2genes.csv") 
```

# look for linolenic acid candidate genes by combining expression/eQTL and SNP/provean data 

```{r}
load("~/F2/for_paper/linolenic_acid/allele_effect_result.Rdata")
allele_effect_result

blast_result_eQTL <- read.delim(file = "~/F2/for_paper/linolenic_acid/reciprocal.blast_eQTL", header = F)
blast_result_eQTL

homeo_blast_eQTL <- 
  c(as.character(blast_result_eQTL$V1), as.character(blast_result_eQTL$V2))

homeo_blast_eQTL
cis_eQTL_linolenic <- allele_effect_result[allele_effect_result$names %in% homeo_blast_eQTL,]
cis_eQTL_linolenic
...
# combine with SNP result to look for potential candidates ... 
<<<<<<< HEAD

blast_result_SNP <- read.delim(file = "~/F2/for_paper/linolenic_acid/reciprocal.blast", header = F)
blast_result_SNP 
=======
>>>>>>> 152f1dad7bc0e940ed36caeff7351591ea292edb
```
