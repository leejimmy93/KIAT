...
test <- 
pheno.parent.H2 %>% 
  filter(PHENOTYPE == "Erucic_acid" & genotype == "Da-Ae")
...
var(phefile.t$Erucic_acid) # VP

# H2 
(var(phefile.t$Erucic_acid) - var(test$value))/var(phefile.t$Erucic_acid)

# oil content 
test <- 
pheno.parent.H2 %>% 
  filter(PHENOTYPE == "Oil_content" & genotype == "Da-Ae")
...
phefile <- read.csv("~/F2/data/QTL_analysis/F2.pheno.csv", header = F, row.names = 1)
phefile.t <- phefile %>% t() %>% as.data.frame() 
var(phefile.t$Crude_oil_contents) # VP

# H2 
(var(phefile.t$Crude_oil_contents) - var(test$value))/var(phefile.t$Crude_oil_contents) 
