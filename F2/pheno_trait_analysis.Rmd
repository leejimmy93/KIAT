...
for (i in colnames(phefile_oil)) {
  hist(phefile_oil[,i], breaks = , col = "grey", border = "black", xlab = "", ylab = "", main = i)
...
# I only have Erucic, oleic and oil content from multiple years, so will consider that 
load("~/F2/for_paper/heritability/parental_oil.Rdata")
tmp.1.1 <- # data from 2016-2017 summarized by Julin 
tmp.1 %>% 
  filter(PHENOTYPE == "Erucic_acid" | PHENOTYPE == "Oil_content" | PHENOTYPE == "Oleic_acid") 

tmp.2.1 <- # data from 2017-2018 
tmp.2 %>% 
  filter(PHENOTYPE == "Erucic_acid" | PHENOTYPE == "Oil_content" | PHENOTYPE == "Oleic_acid") 

tmp.1.1$year <- rep("2016", nrow(tmp.1.1))
tmp.2.1$year <- rep("2017", nrow(tmp.2.1))

pheno.parent.H2 <- rbind(tmp.1.1, tmp.2.1) 
pheno.parent.H2$variable <- gsub("ID_505_K200", "Da-Ae", pheno.parent.H2$variable)
pheno.parent.H2$variable <- gsub("ID_505_K201", "Da-Ol-1", pheno.parent.H2$variable)
pheno.parent.H2$variable <- gsub("K200", "Da-Ae", pheno.parent.H2$variable)
pheno.parent.H2$variable <- gsub("K201", "Da-Ol-1", pheno.parent.H2$variable)

colnames(pheno.parent.H2)[2] <- "genotype"

### erucic 
test <- 
pheno.parent.H2 %>% 
  filter(PHENOTYPE == "Oil_content")

test
lme <- lmer(as.numeric(value) ~ (1|genotype),data=test)
summary(lme) 
var <- as.data.frame(VarCorr(lme)) 
var # residual is the environmental effect  

lme2 <- lmer(as.numeric(value) ~ (1|genotype),data=test)
summary(lme) 
var <- as.data.frame(VarCorr(lme)) 
var # residual is the environmental effect  

phefile <- read.csv("~/F2/data/QTL_analysis/F2.pheno.csv", header = F, row.names = 1)
phefile.t <- phefile %>% t() %>% as.data.frame() 

lme.2 <- lmer(as.numeric(as.character(Oleic_acid)) ~ (1|as.character(id)), data = phefile.t, control=lmerControl(check.nobs.vs.nlev = "ignore",check.nobs.vs.rankZ = "ignore", check.nobs.vs.nRE="ignore")) 

sum(as.data.frame(VarCorr(lme.2))$vcov) 

# oil content 
var(as.numeric(as.character(phefile.t$Crude_oil_contents))) 
# understand how to get heritability for this dataset 
...
# oil trait 
pheno.parent.oil <- read.csv("~/Desktop/F2_paper/data/FA_Da-ae_Da-ol.csv", header = T, as.is = T)
colnames(pheno.parent.oil) <- as.character(pheno.parent.oil[1,])
pheno.parent.oil <- pheno.parent.oil[-1,] 

colnames(pheno.parent.oil) <- gsub("([[:print:]]+)(\\()([[:print:]]+)", "\\1", colnames(pheno.parent.oil))
colnames(pheno.parent.oil) <- gsub("\\ ", "\\_", colnames(pheno.parent.oil))  
colnames(pheno.parent.oil) <- gsub("\\_$", "", colnames(pheno.parent.oil))
colnames(pheno.parent.oil)[2] <- "Name"
pheno.parent.oil[1,2] <- "Da-ae"
pheno.parent.oil[,5:22] <- sapply(colnames(pheno.parent.oil)[5:22], function(i) as.numeric(pheno.parent.oil[,i])) 
pheno.parent.oil
...
pheno.parent.oil$PHENOTYPE <- gsub("cis-11-Eicosenoic_acid", "cis_11_Eicosenoic_acid", pheno.parent.oil$PHENOTYPE)
pheno.parent.oil$PHENOTYPE <- gsub("Palmitoliec_aicd", "Palmitoliec_acid", pheno.parent.oil$PHENOTYPE)
pheno.parent.oil$genotype <- gsub("Da-ae", "Da-Ae", pheno.parent.oil$genotype)
pheno.parent.oil$genotype <- gsub("Da-ol1", "Da-Ol-1", pheno.parent.oil$genotype) 

final_parent <- 
  final_parent_no_oil %>% 
  rbind(pheno.parent.oil) 
...
# second method, use parental data only to calculate heritability 
heritability_2 <- 
sapply(unique(final_parent$PHENOTYPE), function(i) {
  test <- final_parent %>% 
  filter(PHENOTYPE == i) 
