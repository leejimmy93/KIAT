...
cis_eQTL.qtl.combined.final <- read.csv(file = "~/F2/output/eQTL/cis_eQTL.qtl.combined.final.csv", row.names = 1)   
number_cis_eQTL <- 
cis_eQTL.qtl.combined.final %>% 
  group_by(trait.y) %>% 
  summarise(count = n()) 

number_cis_eQTL[grep("acid|content", number_cis_eQTL$trait.y),]$count %>% sum() 
