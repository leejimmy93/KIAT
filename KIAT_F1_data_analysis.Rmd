...
length(vcf.F1.flowering$QUAL) # a total of 2543959 SNPs 
sum(vcf.F1.flowering$QUAL>20) / length(vcf.F1.flowering$QUAL) # 73.1% are above QUAL score of 20, which means less than 0.01 probability that it isn't polymorphic 

hist(vcf.F1[vcf.F1.flowering$QUAL<20,]$QUAL) 
hist(vcf.F1[vcf.F1.flowering$QUAL<1,]$QUAL)

# subset the data to keep positions where the quality score is 40 or higher 
vcf.F1.HQ.flowering <- vcf.F1[vcf.F1.flowering$QUAL>40,]
dim(vcf.F1.HQ.flowering) # 1628615      41  
sum(vcf.F1.flowering$QUAL>40) / length(vcf.F1.flowering$QUAL) # 64% of SNPs were retained with QUAL > 40
...
vcf.F1.HQ.filtered.1 <- vcf.F1.HQ.filtered[((vcf.F1.HQ.filtered$Ae_gt=="1/1" & vcf.F1.HQ.filtered$Ol_gt=="0/0") | (vcf.F1.HQ.filtered$Ae_gt=="0/0" & vcf.F1.HQ.filtered$Ol_gt=="1/1")),] 
dim(vcf.F1.HQ.filtered.1) # 127019 41 
nrow(vcf.F1.HQ.filtered.1)/nrow(vcf.F1.HQ.filtered) # 28% 
ftable(vcf.F1.HQ.filtered.1[,c("414F1_young_gt","415F1_young_gt", "Ae_gt", "Ol_gt")]) # need stats for this... 
