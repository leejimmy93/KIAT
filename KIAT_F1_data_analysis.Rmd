...
A.ID
rownames(A.ID) <- A.ID$V1

A <- as.data.frame(vstMat.F1[(rownames(A.ID)),])
A
dim(A)
...
B.ID
rownames(B.ID) <- B.ID$V1

which(rownames(vstMat.F1) %in% rownames(B.ID))
B <- as.data.frame(vstMat.F1[(c(which(rownames(vstMat.F1) %in% rownames(B.ID)))),])
B
summary(rownames(B) %in% rownames(B.ID))
identical(rownames(B), rownames(B.ID))
dim(B)
...
C.ID
rownames(C.ID) <- C.ID$V1

C <- as.data.frame(vstMat.F1[(rownames(C.ID)),])
C
dim(C)
...
D.ID
rownames(D.ID) <- D.ID$V1

D <- as.data.frame(vstMat.F1[(rownames(D.ID)),])
D
dim(D)
...
E.ID
rownames(E.ID) <- E.ID$V1

which(rownames(vstMat.F1) %in% rownames(E.ID))
E <- as.data.frame(vstMat.F1[(c(which(rownames(vstMat.F1) %in% rownames(E.ID)))),])
E
summary(rownames(E) %in% rownames(E.ID))
identical(rownames(E), rownames(E.ID))
dim(E)
