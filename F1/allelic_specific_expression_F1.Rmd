...
For genes that show DE but not different ASE, there can be several different conditions:

1) non-genetic, which is due to maternal effect, ie. the maternal environment effect
2) genetic, genes which show different ASE are transcription factors which affect the expression of other genes, although the affected genes don'g show different ASE. (FLC example)
