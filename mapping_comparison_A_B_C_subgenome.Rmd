...
1) map flower data from Da-Ol-1 to B.napus & B.napus + B subgenome, 0 mismatch for both, checking mapping result  

2) extract uniquely mapped reads from both mapping result 

3) calculate read depth for each position  

4) subset to each chromosome & calculate average depth for a window size of 1000bp 

5) get the difference in read depth between ABC VS AC mapped reads in 10,000 bp window size 
