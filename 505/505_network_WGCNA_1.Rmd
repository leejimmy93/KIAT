...
vstMat.505.batch.corrected.largeVar<-
  ...
  filter(CV>18) %>% # only the last quantile was left for co-expression analysis 
  dplyr::select(-CV, -variance) 

rownames(vstMat.505.batch.corrected.largeVar) <- 
  rownames(vstMat.505.batch.corrected[vstMat.505.batch.corrected$CV >18,])

dim(vstMat.505.batch.corrected.largeVar) # 10788   131  
...
table(dynamicMods)
table(dynamicMods) %>% length() # 35 modules 
...
MEs = mergedMEs # 30 modules 

save(MEs, moduleColors, file = "~/505/network_analysis/output/MEs_505_unsigned.Rdata")
```
...
dynamicMods <- cutreeDynamic(dendro = geneTree, distM = dissTOM,
                             ...
                             minClusterSize = minModuleSize);
table(dynamicMods)
table(dynamicMods) %>% length() # 32 modules 
...
moduleLabels = match(moduleColors, colorOrder)-1;
MEs = mergedMEs
moduleColors.signed <- moduleColors
MEs.signed <- MEs

save(moduleColors.signed, MEs.signed, file = "~/505/network_analysis/output/MEs_505_signed.Rdata")
``` 
...
# check "brown4" module 
vstMat.505.white <- 
  vstMat.505.batch.corrected.largeVar[moduleColors == "white",] %>% 
  t() %>% as.data.frame()
...
vstMat.505.white %>% 
  ...
  melt() %>% 
  ggplot() + 
  geom_line(mapping = aes(x = Var1, y = Var2)) 

pheatmap(as.matrix(t(vstMat.505.white)),  
