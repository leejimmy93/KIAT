...
# re-run GWAS for late silique dataset 
```{r}
...
match(colnames(late_silique_92_common), colnames(leaf_92_common))
colnames(late_silique_92_common_sorted)
colnames(leaf_92_common)

order <- colnames(leaf_92_common) 
colnames(late_silique_92_common)
match(leaf_92_common, late_silique_92_common)

common_92 <- rbind(leaf_92_common, late_silique_92_common)
...
common_92 %>% 
  select(-tissue, -index)
write.table(tmp3, sep="\t", "~/505/leaf_VS_late_silique/tmp3.hmp", row.names = F) 
...
before <- leaf_92_common
after <- sig_leaf_other_leaf
