...
# display GWAS result #### this is the final result that I used for presentation 
```{r}
...
### overlay current result with previous finding 
```{r}
...
save(SNP.Erucic.Oleic.unique, file =  "~/505/output/131_sample/SNP.Erucic.Oleic.Unique.Rdata")
...
load("~/505/output/131_sample/SNP.Erucic.Oleic.Unique.Rdata")
...
######## this part is not consistent with the info below because new SNP dataset were used for GWAS... 
...
grep("SN|flower_50|bolt_50|fruition|oil", colnames(data.all), value=T)
...
pheno.505.six.long <- 
pheno.505.six %>% 
  gather(key="Taxa", value="value") 

colnames(pheno.505.six.long)[2] <- "trait"

p.phenotype <- 
pheno.505.six.long %>%
  ggplot() + 
  geom_histogram(aes(x=value, fill=trait)) +
  facet_wrap(~trait, nrow=2, ncol=3) + 
  theme(legend.position="none")

ggsave(p.phenotype, filename = "~/505/output/figure/131_sample/p.phenotype.png", height = 8, width = 12)
...
SNP <- GRanges(seqnames = Rle(SNP.time.unique$CHROM), ranges = IRanges(start = SNP.time.unique$POS, end = SNP.time.unique$POS), ID = paste(SNP.time.unique$CHROM, SNP.time.unique$POS, sep = "_"))
SNP
...
SNP_gene_final_fxn$flowering_time <- SNP_gene_final_fxn$SNP.ID %in% flowering_time[flowering_time$P.value < 0.00001,]$SNP

SNP_gene_final_fxn$bolting_time <- SNP_gene_final_fxn$SNP.ID %in% bolting_time[bolting_time$P.value < 0.00001,]$SNP 

colnames(SNP_gene_final_fxn) <- c("gene_ID", "snp_ID", "CHROM", "POS", "Arabidopsis_homolog", "function", "flowering_time", "bolting_time")
View(SNP_gene_final_fxn) 
nrow(SNP_gene_final_fxn) # 204 

write.csv(SNP_gene_final_fxn, file = "~/505/output/131_sample/GWAS_SNP_gene_fxn.csv")

# as a summary, 6 SNPs in total, 5 for bolting time, 1 for flowering time
########### come to this stage... 
``` 
...
SNP_gene_final_fxn <- read.csv("~/505/output/131_sample/GWAS_SNP_gene_fxn.csv")
head(SNP_gene_final_fxn)
dim(SNP_gene_final_fxn) # 6 9   

SNP_gene_final_fxn.2 <- merge(SNP_gene_final_fxn, sig_SNP_ann, by="snp_ID")
SNP_gene_final_fxn.2 
View(SNP_gene_final_fxn.2) 
write.csv(SNP_gene_final_fxn.2, "~/505/output/131_sample/SNP_gene_final_fxn.flowering_bolting.csv")
save(SNP_gene_final_fxn.2, file="~/505/output/131_sample/SNP_gene_final_fxn.flowering_bolting.Rdata")
```

I don't see obvious flowering time or bolting time related genes, maybe look for genes in the bolting time region 
```{r}
SNP.range <- GRanges(seqnames = Rle("chrA04"), ranges = IRanges(start = 17445226, end = 18422534))
SNP.range
...
# GO enrichment of genes in range 
```{r}
source("/Users/ruijuanli/Desktop/Brassica_project/KIAT_RNA_seq/analysis/function_BnRNAseq.R")
load("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/131_sample/SNP_range_gene.df.Rdata")
GOseq.Bn.ORA(SNP_range_gene.df$names) # no enriched GO terms 
head(SNP_range_gene.df) 

```

# annotate genes in this range 
```{r}
SNP_gene_final <- SNP_range_gene.df[,c("SNP.range.seqnames", "SNP.range.start", "SNP.range.end", "genes.start", "genes.end", "names")]
head(SNP_gene_final)
...
View(SNP_gene_final_fxn) 
nrow(SNP_gene_final_fxn) # 229

write.csv(SNP_gene_final_fxn, file = "~/505/output/131_sample/SNP_range_gene_bolting_time.csv")
``` 

# batch F 
```{r}
# trimming & mapping was done on whitney 
# mapping result analysis 
source("/Users/ruijuanli/Desktop/Brassica_project/KIAT_RNA_seq/analysis/function_BnRNAseq.R")

# sample description 
sample_des_f <- read.csv("~/505/data/phenotype/batch_f.csv", header = T) 
sample_des_f_sub <- sample_des_f[,c("Sample.ID","TotalReads")]
head(sample_des_f_sub)
sample_des_f_sub$Sample.ID <- gsub(" ", "_", sample_des_f_sub$Sample.ID)
sample_des_f_sub$Sample.ID

# mapping result
mapping_result <- read.table("~/505/data/late_silique/Star_Stats.tab", header = T)
head(mapping_result)
mapping_result$Sample.ID <- gsub("(Sample_)([[:print:]])", "\\2", mapping_result$Sample) 

mapping_result_505 <- merge(sample_des_f_sub, mapping_result, by="Sample.ID")

### calculate average 
mean(mapping_result_505$TotalReads/2) # 19736643

# number of total reads 
library(ggplot2) 
p.total.reads <- ggplot(data=mapping_result_505)
p.total.reads <- p.total.reads + geom_bar(aes(x=reorder(Sample.ID, TotalReads/2), y=TotalReads/2), stat = "identity")
p.total.reads <- p.total.reads + labs(list(title = "", x = "", y = "Number of raw reads"))
p.total.reads <- p.total.reads + theme(axis.text.x = element_text(angle = 90, size = 8)) 
p.total.reads   
ggsave(p.total.reads, filename = "~/505/output/figure/total_reads.png", height = 8, width = 11) 
...
mapping_result_505_sub 
dim(mapping_result_505_sub) # 37 5 

mapping_result_505_sub.long <- melt(mapping_result_505_sub, id.vars = c("Sample.ID"))
...
# p.total.reads <- p.total.reads + theme(axis.text.x = element_text(angle = 90, size = 8)) 
pl.mapping.indi.c
ggsave(pl.mapping.indi.c, filename = "~/505/output/figure/indi.png", height = 8, width = 11) 

# look at histogram of HQ reads 
colnames(mapping_result_505)
p.HQ.perc <- ggplot(data=mapping_result_505)
p.HQ.perc <- p.HQ.perc + geom_histogram(aes(x=mapping_result_505$Number_Input_Reads/(mapping_result_505$TotalReads/2)), binwidth = 0.01) 
p.HQ.perc <- p.HQ.perc + labs(list(title = "histogram of HQ reads percentage", x = "Percent of high qulity reads", y = "number of samples"))
p.HQ.perc 
# ggsave(p.HQ.perc, filename = "~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/figure/HQ_hist.png", height = 8, width = 11) 
...
# hisogram of multimapped & too many reads 
p.multi.too.many.mapping <- ggplot(data=mapping_result_505)
p.multi.too.many.mapping <- p.multi.too.many.mapping + geom_histogram(aes(x=mapping_result_505$Percent_Multi_Mapped+mapping_result_505$Percent_Too_Many_Multi_Mapped), binwidth = 2.5) 
p.multi.too.many.mapping <- p.multi.too.many.mapping + labs(list(title = "", x = "Percent of multi & too many mapped reads", y = "number of samples")) 
p.multi.too.many.mapping

# histogram of multi mapped 
p.multi.mapping <- ggplot(data=mapping_result_505)
p.multi.mapping <- p.multi.mapping + geom_histogram(aes(x=mapping_result_505$Percent_Multi_Mapped), binwidth = 2.5) 
p.multi.mapping <- p.multi.mapping + labs(list(title = "histogram of multi mapped reads percentage", x = "Percent of multi mapped reads", y = "number of samples"))
p.multi.mapping
# ggsave(p.multi.mapping, filename = "~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/figure/multi_mapped.png", height = 8, width = 11) 
...
# histogram of too many mapped 
p.toomany.mapping <- ggplot(data=mapping_result_505) 
p.toomany.mapping <- p.toomany.mapping + geom_histogram(aes(x=mapping_result_505$Percent_Too_Many_Multi_Mapped), binwidth = 2.5) 
p.toomany.mapping <- p.toomany.mapping + labs(list(title = "histogram of too many mapped reads percentage", x = "Percent of toomany mapped reads", y = "number of samples"))
p.toomany.mapping
# ggsave(p.toomany.mapping, filename = "~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/figure/toomany_mapped.png", height = 8, width = 11) 
...
# mapping stats 
colnames(mapping_result_505)
mapping_result_505$batch <- rep("f", nrow(mapping_result_505))
# average percentage of HQ, unique, multi, toomany, unmapped 
average.mapping <- aggregate(mapping_result_505[,c("TotalReads","Number_Input_Reads", "Percent_Unique_Mapped", "Percent_Multi_Mapped", "Percent_Too_Many_Multi_Mapped", "Percent_Unmapped_Too_Short")], list(mapping_result_505$batch), mean)
...
p.average.mapping <- p.average.mapping + geom_text(data = average.mapping.long, aes(x=variable, y=value, label=factor(round(value,0)))) 
p.average.mapping
ggsave(p.average.mapping, filename =  "~/505/output/figure/average_mapping.png", height = 6, width = 9) 
...
<<<<<<< HEAD
# is batch F duplicate or new lines for 505? 
```{r}
# sample description 
# is batch F duplicate or new lines for 505? 
# sample description 
sample_des_f <- read.csv("~/505/data/phenotype/batch_f.csv", header = T) 
sample_des_f_sub <- sample_des_f[,c("Sample.ID","TotalReads", "Name")]
head(sample_des_f_sub)

# rep1 
sample_des_c <- read.csv("~/505/data/phenotype/batch_c.csv", header = T) 
sample_des_c_sub <- sample_des_c[,c("Sample.ID","TotalReads","Name")]
sample_des_d <- read.csv("~/505/data/phenotype/batch_d.csv", header = T)
sample_des_d_sub <- sample_des_d[,c("Sample.ID","TotalReads","Name")]
head(sample_des_d_sub)
sample_des_e <- read.csv("~/505/data/phenotype/batch_e.csv", header = T) 
sample_des_e_sub <- sample_des_c[,c("Sample.ID","TotalReads","Name")]

rep1 <- rbind(sample_des_c_sub, sample_des_d_sub, sample_des_e_sub)
<<<<<<< HEAD
rep1$Name  
```

### get the chromosome name for chromsome ID from GWAS result 
```{r}
Erucic_acid <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/myGAPIT/late_silique/GAPIT..Erucic_acid.GWAS.Results.csv", header = T)
Erucic_acid$Chrom_name <- gsub("(S)([[:print:]]+)(_)([[:digit:]]+)", "\\2", Erucic_acid$SNP)   

Chrom_name <- 
Erucic_acid %>% 
  dplyr::select(Chromosome, Chrom_name) %>% 
  unique()
Chrom_name 
rep1$Name
```

### GWAS on all fatty acid composition and oil content 
```{r}
...
png("~/505/output/131_sample/pheno_fatty_acid_pair.png", width = 30, height = 30, units = "in", res = 300)  
pheno_data_505 %>% 
  dplyr::select(-Taxa) %>% 
  pairs()
dev.off()   
...
### GWAS on all fatty acid traits 
```{r}
# https://github.com/leejimmy93/KIAT/blob/master/505/GWAS_505_late_silique_fatty_acid_all.R
# generate mahattan plot 

# Oil_content", "Myristic_acid", "Pentadecanoic_acid", "Palmitic_acid", "Palmitoliec_aicd", "Heptadecanoic_acid", "Stearic_acid", "Oleic_acid", "vaccenic_acid", "Linoleic_acid", "Arachidic_acid", "cis_11_Eicosenoic_acid", "Linolenic_acid", "Behenic_acid", "Erucic_acid" These many traits have GWAS results, the other three don't because of two few data points. 
...
fatty_acid_all <- list(Oil_content, Myristic_acid, Pentadecanoic_acid, Palmitic_acid, Palmitoliec_acid, Heptadecanoic_acid, Stearic_acid, Oleic_acid, vaccenic_acid, Linoleic_acid, Arachidic_acid, cis_11_Eicosenoic_acid, Linolenic_acid, Behenic_acid, Erucic_acid)

length(fatty_acid_all) # 15 
names(fatty_acid_all) <- c("Oil_content", "Myristic_acid", "Pentadecanoic_acid", "Palmitic_acid", "Palmitoliec_aicd", "Heptadecanoic_acid", "Stearic_acid", "Oleic_acid", "vaccenic_acid", "Linoleic_acid", "Arachidic_acid", "cis_11_Eicosenoic_acid", "Linolenic_acid", "Behenic_acid", "Erucic_acid")
fatty_acid_all[["Oil_content"]] %>% head()
colnames(fatty_acid_all[["Oil_content"]])
# plot and save all plot 

png("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/505/output/figure/131_sample/all_fatty_acid_1.png", width=12, height=10, units="in", res=300)
par(mfrow=c(3,1)) 

colnames(fatty_acid_all[["Oil_content"]])[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(fatty_acid_all[["Myristic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(fatty_acid_all[["Pentadecanoic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")

manhattan((fatty_acid_all[["Oil_content"]]), main = "Oil_content", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
manhattan((fatty_acid_all[["Myristic_acid"]]), main = "Myristic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
manhattan((fatty_acid_all[["Pentadecanoic_acid"]]), main = "Pentadecanoic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
dev.off() 

png("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/505/output/figure/131_sample/all_fatty_acid_2.png", width=12, height=10, units="in", res=300)
par(mfrow=c(3,1)) 

colnames(fatty_acid_all[["Palmitic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(fatty_acid_all[["Palmitoliec_aicd"]])[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(fatty_acid_all[["Heptadecanoic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")

manhattan((fatty_acid_all[["Palmitic_acid"]]), main = "Palmitic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
manhattan((fatty_acid_all[["Palmitoliec_aicd"]]), main = "Palmitoliec_aicd", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
manhattan((fatty_acid_all[["Heptadecanoic_acid"]]), main = "Heptadecanoic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
dev.off()  

png("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/505/output/figure/131_sample/all_fatty_acid_3.png", width=12, height=10, units="in", res=300)
par(mfrow=c(3,1)) 

colnames(fatty_acid_all[["Stearic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(fatty_acid_all[["Oleic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(fatty_acid_all[["vaccenic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")

manhattan((fatty_acid_all[["Stearic_acid"]]), main = "Stearic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
manhattan((fatty_acid_all[["Oleic_acid"]]), main = "Oleic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
manhattan((fatty_acid_all[["vaccenic_acid"]]), main = "vaccenic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
dev.off()  

png("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/505/output/figure/131_sample/all_fatty_acid_4.png", width=12, height=10, units="in", res=300)
par(mfrow=c(3,1)) 

colnames(fatty_acid_all[["Linoleic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(fatty_acid_all[["Arachidic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(fatty_acid_all[["cis_11_Eicosenoic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")

manhattan((fatty_acid_all[["Linoleic_acid"]]), main = "Linoleic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
manhattan((fatty_acid_all[["Arachidic_acid"]]), main = "Arachidic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
manhattan((fatty_acid_all[["cis_11_Eicosenoic_acid"]]), main = "cis_11_Eicosenoic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
dev.off()   

png("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/505/output/figure/131_sample/all_fatty_acid_5.png", width=12, height=10, units="in", res=300)
par(mfrow=c(3,1)) 

colnames(fatty_acid_all[["Linolenic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(fatty_acid_all[["Behenic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(fatty_acid_all[["Erucic_acid"]])[1:4] <- c("SNP", "CHR", "BP", "P")

manhattan((fatty_acid_all[["Linolenic_acid"]]), main = "Linolenic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
manhattan((fatty_acid_all[["Behenic_acid"]]), main = "Behenic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
manhattan((fatty_acid_all[["Erucic_acid"]]), main = "Erucic_acid", ylim = c(0, 8), cex = 0.6,
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F)
dev.off()    
```

### fit a model for the A08 and C03 loci from Linolenic acid, see whether there is interaction or not. 
```{r}
Linolenic_acid <- read.csv("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/505/output/myGAPIT/late_silique_131_sample/fatty_acid_all/GAPIT..Linolenic_acid.GWAS.Results.csv", header = T, stringsAsFactors = F)

Oleic_acid <- read.csv("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/505/output/myGAPIT/late_silique_131_sample/fatty_acid_all/GAPIT..Oleic_acid.GWAS.Results.csv", header = T)

Erucic_acid <- read.csv("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/505/output/myGAPIT/late_silique_131_sample/fatty_acid_all/GAPIT..Erucic_acid.GWAS.Results.csv", header = T)

colnames(Linolenic_acid)
Linolenic_acid$P.value # data is already sorted based on p-value, pick A08 marker and C03 marker which have the smallest p-value 
...
C03_marker <- 
Linolenic_acid %>% 
  filter(Chromosome == "26") %>% 
  head(4) %>% 
  dplyr::select(SNP) %>% 
  as.character() 

# I want to combine Linolenic, Erucic, and Oleic acid analysis altogether, so chose two peak markers which are significant for all three fatty acid compositions, also these two markers are present in Da-Ae and Da-Ol-1 SNP set. 
A08_marker <- "SA08_10597112" 
C03_marker <- "SC03_56259024" 

# check to see whether these two SNPs are present in Da-Ae and Da-Ol-1 data 
Ae_Ol <- read.csv("~/SNP_parent/result/vcf.Ae.Ol.intersect.final.csv", stringsAsFactors = F)
Ae_Ol %>% head() 
Ae_Ol$SNP_ID <- paste(Ae_Ol$CHROM, Ae_Ol$POS, sep = "_")
Ae_Ol$SNP_ID <- gsub("chr", "S", Ae_Ol$SNP_ID)

# extract gt info for these two markers 
geno_505_hmp <- read.table("~/505/vcf_late_silique_131_sample/combined/505_filtered_het_0.2.recode.hmp.txt", head=FALSE, stringsAsFactors = F) 
dim(geno_505_hmp) # 174398    142 
geno_505_hmp[1:10, 1:10]

rownames(geno_505_hmp) <- geno_505_hmp$V1
...
dim(geno_505_hmp.2)
geno_505_hmp.2 <- geno_505_hmp.2[,-1] 
colnames(geno_505_hmp.2) <- geno_505_hmp.2[1,] 
geno_505_hmp.2 <- geno_505_hmp.2[-1,]
dim(geno_505_hmp.2) # 174397    131
geno_505_hmp.2[1:10, 1:10] 

geno_A08_C03 <- 
geno_505_hmp.2[(rownames(geno_505_hmp.2) %in% A08_marker) | (rownames(geno_505_hmp.2) %in% C03_marker),] 
rname <- rownames(geno_A08_C03)
geno_A08_C03 <- 
IUPAC.code.reverse(temp2 = geno_A08_C03)

geno_A08_C03$SNP_ID <- rname
geno_A08_C03

geno_A08_C03# polarize againt Da-Ae and Da-Ol-1 gt 
Ae_Ol_A08_C03 <- Ae_Ol[(Ae_Ol$SNP_ID %in% A08_marker) | (Ae_Ol$SNP_ID %in% C03_marker),] 
Ae_Ol_A08_C03

geno_A08_C03_Ae_Ol <- 
geno_A08_C03 %>% 
  left_join(Ae_Ol_A08_C03)  

# get genotype for Da-Ae and Da-Ol-1 
geno_A08_C03_Ae_Ol$Ae.gt <- 
ifelse(geno_A08_C03_Ae_Ol$Ae.gt == "0/0", paste(geno_A08_C03_Ae_Ol$REF, geno_A08_C03_Ae_Ol$REF, sep = ""), paste(geno_A08_C03_Ae_Ol$ALT, geno_A08_C03_Ae_Ol$ALT, sep = ""))

geno_A08_C03_Ae_Ol$Ol.gt <- 
ifelse(geno_A08_C03_Ae_Ol$Ol.gt == "0/0", paste(geno_A08_C03_Ae_Ol$REF, geno_A08_C03_Ae_Ol$REF, sep = ""), paste(geno_A08_C03_Ae_Ol$ALT, geno_A08_C03_Ae_Ol$ALT, sep = ""))

geno_A08_C03_Ae_Ol
# get genotype for 
geno_A08_C03_Ae_Ol_new <- data.frame()

for (i in colnames(geno_A08_C03_Ae_Ol)[1:131]){
  # print(i)   
  for (j in 1:nrow(geno_A08_C03_Ae_Ol)){
    print(j)
    if (geno_A08_C03_Ae_Ol[j,i] == "NN"){
      geno_A08_C03_Ae_Ol_new[j,i] = "NN"
    } else if (geno_A08_C03_Ae_Ol[j,i] != geno_A08_C03_Ae_Ol[j,"Ae.gt"] & geno_A08_C03_Ae_Ol[j,i] != geno_A08_C03_Ae_Ol[j,"Ol.gt"]){
      geno_A08_C03_Ae_Ol_new[j,i] = "AB"
    } else if (geno_A08_C03_Ae_Ol[j,i] == geno_A08_C03_Ae_Ol[j,"Ae.gt"]){
      geno_A08_C03_Ae_Ol_new[j,i] = "AA"
    } else {
      geno_A08_C03_Ae_Ol_new[j,i] = "BB" 
    }
  }
}  

rownames(geno_A08_C03_Ae_Ol_new) <- geno_A08_C03_Ae_Ol$SNP_ID

geno_A08_C03_Ae_Ol_new <- as.data.frame(t(geno_A08_C03_Ae_Ol_new))
geno_A08_C03_Ae_Ol_new$Taxa <- rownames(geno_A08_C03_Ae_Ol_new)
geno_A08_C03_Ae_Ol_new$Taxa <- gsub("\\.","\\-", rownames(geno_A08_C03_Ae_Ol_new))
geno_A08_C03_Ae_Ol_new$Taxa <- gsub("X", "", geno_A08_C03_Ae_Ol_new$Taxa)
geno_A08_C03_Ae_Ol_new$Taxa

# combine with phenotype data 
load("~/505/genomic_prediction/data/pheno_data_fatty_acid_all.Rdata")
pheno_data_505_Linolenic <- 
pheno_data_505 %>% 
  dplyr::select(Taxa,Linolenic_acid, Erucic_acid, Oleic_acid)

data_three_acid <- 
geno_A08_C03_Ae_Ol_new %>% 
  left_join(pheno_data_505_Linolenic) 

### start modeling 
summary(data_three_acid)
data_three_acid$Taxa <- as.factor(data_three_acid$Taxa)
data_three_acid$SA08_10597112 %>% class()
data_three_acid$SC03_56259024 %>% class()

### try lmerTest 
library(lmerTest)
# mixed effect linear model selection (taxa as random effect), using all orignal data scale 
data_three_acid$SA08_10597112 <- relevel(data_three_acid$SA08_10597112, ref = "AA")
data_three_acid$SC03_53911912 <- relevel(data_three_acid$SC03_56259024, ref = "AA")
lm.Linolenic <- lm(Linolenic_acid ~ SA08_10597112 * SC03_56259024, data = data_three_acid)
lm.Erucic <- lm(Erucic_acid ~ SA08_10597112 * SC03_56259024, data = data_three_acid)
lm.Oleic <- lm(Oleic_acid ~ SA08_10597112 * SC03_56259024, data = data_three_acid)

summary.lm.Linolenic <- summary(lm.Linolenic) 
summary.lm.Erucic <- summary(lm.Erucic)
summary.lm.Oleic <- summary(lm.Oleic)

anova(lm.Linolenic) # assessing significance of fixed effects 
anova(lm.Erucic)
anova(lm.Oleic)
lm.Linolenic$coefficients 

### if I try in brms  
library(brms)
data_three_acid$SA08_10597112 <- relevel(data_three_acid$SA08_10597112, ref = "AA")
data_Linolenic$SC03_56259024 <- relevel(data_three_acid$SC03_56259024, ref = "AA") 

m.Linolenic.brms <- brm(Linolenic_acid ~  SA08_10597112 * SC03_56259024, 
            data = data_Linolenic,
            prior = c(set_prior("normal(0,10)",class="b")) # sets prior for all b coefficients  
) 

summary(m.Linolenic.brms)
fixef(m.Linolenic.brms)  

# plot out the lmerTest result 
allele.effect.result <- function(data){ # data is summary.lm.Linolenic$coefficients 
  tmp <- as.data.frame(data[,c("Estimate", "Std. Error")])
  colnames(tmp) <- c("mean", "SE")
  tmp <- tmp[-(grep("NN", rownames(tmp))),]
  
  tmp$SA08_10597112 <- c("AA", "AB", "BB", "AA", "AA", "AB", "BB", "AB", "BB")
  tmp$SC03_56259024 <- c("AA", "AA", "AA", "AB", "BB", "AB", "AB", "BB", "BB")
  
  tmp$mean[9] = tmp$mean[1]+tmp$mean[3]+tmp$mean[5]+tmp$mean[9]
  tmp$mean[8] = tmp$mean[1]+tmp$mean[2]+tmp$mean[5]+tmp$mean[8]
  tmp$mean[7] = tmp$mean[1]+tmp$mean[3]+tmp$mean[4]+tmp$mean[7]
  tmp$mean[6] = tmp$mean[1]+tmp$mean[2]+tmp$mean[4]+tmp$mean[6]
  tmp$mean[5] = tmp$mean[1]+tmp$mean[5]
  tmp$mean[4] = tmp$mean[1]+tmp$mean[4]
  ...
  tmp$SA08_10597112 <- factor(tmp$SA08_10597112, c("AA", "AB", "BB"))
  tmp$SC03_56259024 <- factor(tmp$SC03_56259024, c("AA", "AB", "BB"))
  
  pl <- 
  tmp %>% 
    ggplot() + 
    geom_line(mapping = aes(x=SC03_56259024, y = mean, color = SA08_10597112, group=SA08_10597112)) 
  return(pl) 
}

pl.allele.effect.Linolenic <- allele.effect.result(summary.lm.Linolenic$coefficients)
pl.allele.effect.Erucic <- allele.effect.result(summary.lm.Erucic$coefficients)
pl.allele.effect.Oleic <- allele.effect.result(summary.lm.Oleic$coefficients) 

library(cowplot)

plot.all<-plot_grid(
  pl.allele.effect.Erucic+theme(legend.position = "none", axis.text.x=element_text(size=8,vjust=0.5))+labs(title="Erucic acid"),
  pl.allele.effect.Oleic+theme(legend.position = "none", axis.text.x=element_text(size=8,vjust=0.5))+labs(title="Oleic acid"),
  pl.allele.effect.Linolenic+theme(legend.position = "none", axis.text.x=element_text(size=8,vjust=0.5))+labs(title="Linolenic acid"),
  ncol=3, nrow = 1,labels=c("","","",""))

plot.all
save_plot("~/505/output/131_sample/allele_effect_plot.all.png", plot.all, ncol = 1, nrow = 3,base_aspect_ratio = 0.8) 
```
...
### check indel effect 
```{r}
# get snpEff annotation for the 505.vcf then extract BnaA08g11130D and BnaC03g65980D related annotations 
BnaA08g11130D <- read.table("~/505/vcf_late_silique_131_sample/combined/BnaA08g11130D", as.is=T,na.strings = ".")
BnaC03g65980D <- read.table("~/505/vcf_late_silique_131_sample/combined/BnaC03g65980D", as.is=T,na.strings = ".")
load("~/505/output/131_sample/SNP.Erucic.Oleic.Unique.Rdata")
SNP.Erucic.Oleic.unique %>% dim() # 235 2 
SNP.Erucic.Oleic.unique %>% colnames() 

FAE1 <- rbind(BnaA08g11130D, BnaC03g65980D) 
FAE1 %>% dim() 

colnames(FAE1)[1:2] <- c("CHROM", "POS")

SNP.Erucic.Oleic.unique %>% 
  semi_join(FAE1) %>% 
  dim() # no common items, all of them were filtered out because of either not being SNP or low quality  
``` 

# test 
```{r}
setwd("~/505/vcf_combine_WGS_RNAseq/RNA_2019/")

######### load data
# genotype data
geno_505_hmp <- read.table("~/505/vcf_late_silique_131_sample/combined/hmp/505_RNA_impute.hmp.txt", head=FALSE)
# get chrom name to numerics
geno_505_hmp$V3 <- as.numeric(geno_505_hmp$V3)
geno_505_hmp[1,3] <- "chrom" 
...
myY <- 
myY %>%
  dplyr::select(Taxa, Oleic_acid, Erucic_acid)
...
library("qqman") 
Erucic_acid <- read.csv("~/505/vcf_combine_WGS_RNAseq/RNA_2019/GAPIT.MLM.Erucic_acid.GWAS.Results.csv", header = T) 

Oleic_acid <- read.csv("~/505/vcf_combine_WGS_RNAseq/RNA_2019/GAPIT.MLM.Oleic_acid.GWAS.Results.csv", header = T)

colnames(Erucic_acid)[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(Oleic_acid)[1:4] <- c("SNP", "CHR", "BP", "P")

png("~/505/WGS/GWAS/2019/test_imputation/test2.png", width=12, height=10, units="in", res=300)
par(mfrow=c(2,1)) 
manhattan(Erucic_acid, main = "Erucic acid", ylim = c(0, 8), cex = 0.6, 
    cex.axis = 0.9, col = c("blue4", "orange3"), supggestiveline = 5, genomewideline = F 
    ) 
...
Oleic_acid_old <- read.csv("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/505/output/myGAPIT/late_silique_131_sample/fatty_acid_all/GAPIT..Oleic_acid.GWAS.Results.csv", header = T)

Erucic_acid_old <- read.csv("/Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/505/output/myGAPIT/late_silique_131_sample/fatty_acid_all/GAPIT..Erucic_acid.GWAS.Results.csv", header = T) 

colnames(Erucic_acid_old)[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(Oleic_acid_old)[1:4] <- c("SNP", "CHR", "BP", "P")

Erucic_acid_old_sig <- 
Erucic_acid_old %>% 
  filter(P < 0.00001)  # 280 

Oleic_acid_old_sig <- 
Oleic_acid_old %>% 
  filter(P < 0.00001)

intersect(Erucic_acid_old_sig[,"SNP"], Erucic_acid_sig[,"SNP"]) %>% length()
intersect(Oleic_acid_old_sig[,"SNP"], Oleic_acid_sig[,"SNP"]) %>% length()
dim(Erucic_acid_old_sig)
dim(Oleic_acid_old_sig)
dim(Erucic_acid_sig)
dim(Oleic_acid_sig)  
```
...
# genotype data: impuated data for GWAS 
geno_505_hmp <- read.table("~/505/vcf_late_silique_131_sample/combined/hmp/505_RNA_impute.hmp.txt", head=FALSE)
