...
# another thing worth check is to see the depth between leaf & silique data, also check the heterzygosity ratio for leaf & late silique data, because my impression is that most inconsistency are due to missing data or het calls 
# different parameters should be used for SNP filtering. 
```{r}
...
# modify all to numeric vectors (t-test only accept numeric value)
late_silique_505_common.t.numeric <- 
apply(late_silique_505_common.t, 2, as.character) %>%
  apply(2, as.numeric) %>% 
  as.data.frame()

rownames(late_silique_505_common.t.numeric) <- rownames(late_silique_505_common.t)

leaf_505_common.t.numeric <- 
apply(leaf_505_common.t, 2, as.character) %>%
  apply(2, as.numeric) %>%
  as.data.frame()

rownames(leaf_505_common.t.numeric) <- rownames(leaf_505_common.t)

## T-test   
significant_SNP

Pvalue.leaf <- data.frame()

for (i in significant_SNP){
  tmp <- t.test(leaf_505_common.t.numeric[,i], pheno_data_505_92_leaf$Erucic_acid) 
  Pvalue.leaf[i,1] <- tmp$p.value
}

Pvalue.late_silique <- data.frame()

for (i in significant_SNP){
  tmp <- t.test(late_silique_505_common.t.numeric[,i], pheno_data_505_92_late_silique$Erucic_acid) 
  Pvalue.late_silique[i,1] <- tmp$p.value
}

colnames(Pvalue.leaf) <- "leaf"
colnames(Pvalue.late_silique) <- "late_silique"

ttest_Pvalues <- merge(Pvalue.leaf, Pvalue.late_silique, by="row.names")
ttest_Pvalues
# I don't think t-test the is test used in GWAS... 
...
vcf <- late_silique_505_common.2.2
dim(vcf)
significant_SNP <- significant_SNP_9
