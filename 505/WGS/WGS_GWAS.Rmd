...
setwd("~/check_low_coverage/bioinformatics/BWA/GWAS") 

geno_505_hmp <- read.table("~/check_low_coverage/bioinformatics/BWA/MAF_0.05/hmp/chrC03_het_0.2.hmp.txt", head=FALSE, stringsAsFactors = F)  
...
library("qqman") 
Erucic_acid <- read.csv("~/check_low_coverage/bioinformatics/BWA/GWAS/GAPIT.MLM.Erucic_acid.GWAS.Results.csv", header = T) 

Oleic_acid <- read.csv("~/check_low_coverage/bioinformatics/BWA/GWAS/GAPIT.MLM.Oleic_acid.GWAS.Results.csv", header = T)

colnames(Erucic_acid)[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(Oleic_acid)[1:4] <- c("SNP", "CHR", "BP", "P")

png("~/check_low_coverage/bioinformatics/BWA/GWAS/test.png", width=12, height=10, units="in", res=300)
par(mfrow=c(2,1)) 
