...
setwd("~/505/WGS/GWAS/2019/test_imputation_2/") 

geno_505_hmp <- read.table("~/505/WGS/vcf_filtered/two_options/hmp/505_WGS_impute_2.hmp.txt", head=FALSE, stringsAsFactors = F)  
...
library("qqman") 
Erucic_acid <- read.csv("~/505/WGS/GWAS/2019/test_imputation_2/GAPIT.MLM.Erucic_acid.GWAS.Results.csv", header = T) 

Oleic_acid <- read.csv("~/505/WGS/GWAS/2019/test_imputation_2/GAPIT.MLM.Oleic_acid.GWAS.Results.csv", header = T)

colnames(Erucic_acid)[1:4] <- c("SNP", "CHR", "BP", "P")
colnames(Oleic_acid)[1:4] <- c("SNP", "CHR", "BP", "P")

png("~/505/WGS/GWAS/2019/test_imputation_2/test.png", width=12, height=10, units="in", res=300)
par(mfrow=c(2,1)) 
