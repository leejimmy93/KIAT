...
mean.depth <- read.table("~/505/vcf_leaf/combined/505_filtered_MAF_Q_depth_missingness.ldepth.mean", header=T)
head(mean.depth)
...
rownames(geno.numeric.df) 
# change ID 
rownames(geno.numeric.df) <- gsub("\\.","\\-", rownames(geno.numeric.df)) 
rownames(geno.numeric.df) <- gsub("X", "", rownames(geno.numeric.df))

load("~/505/output/common.lines.Rdata")
geno.numeric.df.common <- geno.numeric.df[rownames(geno.numeric.df) %in% common.lines$Taxa.x,]
# check to make sure I get the right subset 
identical(rownames(geno.numeric.df.common)[order(rownames(geno.numeric.df.common))], common.lines$Taxa.x[order(common.lines$Taxa.x)])
...
# p.mds.country <- p.mds.country + facet_wrap(~Sample.Description.Type)
p.mds.country.2
ggsave(p.mds.country.2, filename="~/505/output/figure/leaf/p.mds.country.2.common.png", height=20, width=20) 
...
load("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/leaf/pheno_data_505_late_silique.R")
load("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/leaf/pheno_data_505_leaf.R")
...
# draw the similar figure for unique lines 




=======
ggsave(P.oil, filename="~/505/output/figure/leaf/P.oil.png", width=6, height=10)

# do seperate graph for unique lines 
common.lines
pheno_data_505[pheno_data_505$Taxa %in% common.lines$Taxa.x,]

pheno.unique.leaf <- pheno_data_505[which(!(pheno_data_505$Taxa %in% common.lines$Taxa.x)),]

pheno_data_505_melt_unique <- melt(pheno.unique.leaf[,2:4])
...
ggsave(P.oil, filename="~/505/output/figure/leaf/P.oil.leaf.unique.png", width=6, height=10)
>>>>>>> b2025ed88eb8ad66cb1c75cbeeaa9861b9b43b3b
...
intersect(lines_leaf_2$Name, lines_late_silique_2$Name) 
save(common_leaf_late_silique, file="~/505/output/common_leaf_late_silique.Rdata") 
# common_leaf_late_silique <- 
  
common.lines <- merge(lines_leaf_2, lines_late_silique_2, by="Name")[,c("Name", "Taxa.x", "Taxa.y")]
common.lines 
save(common.lines, file="~/505/output/common.lines.Rdata")
...
Erucic_acid_leaf <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/myGAPIT/leaf/missing_0.5/GAPIT..Erucic_acid.GWAS.Results.csv", header = T) 

Oil_content_leaf <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/myGAPIT/leaf/missing_0.5/GAPIT..Oil_content.GWAS.Results.csv", header = T)

Oleic_acid_leaf <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/myGAPIT/leaf/missing_0.5/GAPIT..Oleic_acid.GWAS.Results.csv", header = T)

Erucic_acid_late_silique <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/myGAPIT/late_silique/GAPIT..Erucic_acid.GWAS.Results.csv", header = T)

Oil_content_late_silique <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/myGAPIT/late_silique/GAPIT..Oil_content.GWAS.Results.csv", header = T)

Oleic_acid_late_silique <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/myGAPIT/late_silique/GAPIT..Oleic_acid.GWAS.Results.csv", header = T)

sum(Erucic_acid_leaf[Erucic_acid_leaf$P.value < 0.00001,]$SNP %in% Erucic_acid_late_silique$SNP)
# 18 are present in the late silique data out of 28 SNPs 

common.SNPs <- Erucic_acid_leaf[which(Erucic_acid_leaf[Erucic_acid_leaf$P.value < 0.00001,]$SNP %in% Erucic_acid_late_silique$SNP),]$SNP 
save(common.SNPs, file = "~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/common.SNPs.Rdata")

dim(Erucic_acid_late_silique[c(Erucic_acid_leaf[Erucic_acid_leaf$P.value < 0.00001,]$SNP),]) 

Erucic_acid_leaf[Erucic_acid_leaf$P.value < 0.00001,]$SNP 
...
write.table(significant_SNP, file = "~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/significant_SNP.txt")
```
...
=======
# cluster analysis for common lines between leaf & late silique using common SNPs 
```{r}
# filter based on individuals 
load("~/505/output/common.lines.Rdata")
write.table(common.lines, file="~/505/leaf_VS_late_silique/common.lines.txt")
# cat common.lines.txt | awk 'BEGIN{FS="\""}{print $8}' > late_silique_common_ID
# vcftools --keep leaf_common_ID --gzvcf ../vcf_leaf_no_pop/combined/missing_0.5/505_filtered.vcf.gz --recode --recode-INFO-all --out 505_filtered_common.vcf.gz 

# index both leaf & late silique vcf files 
# bgzip -c 505_filtered_sorted.vcf > 505_filtered_sorted.vcf.gz
# tabix -p vcf 505_filtered_sorted.vcf.gz

# 1) filter based on position  
# cat 505_filtered_common_late_silique.vcf | grep -v "#" | awk '{print $1 "\t" $2}' > POS_late_silique
# cat 505_filtered_common_leaf.vcf | grep -v "#" | awk '{print $1 "\t" $2}' > POS_leaf
# 
# vcftools --positions POS_late_silique --vcf 505_filtered_common_leaf.vcf --recode --recode-INFO-all --out 505_filtered_common_leaf_2.vcf
# vcftools --positions POS_leaf --vcf 505_filtered_common_late_silique.vcf --recode --recode-INFO-all --out 505_filtered_common_late_silique_2.vcf

# index 
# bgzip -c 505_filtered_common_leaf_2.vcf.recode.vcf > 505_filtered_common_leaf_2.vcf.gz
# bgzip -c 505_filtered_common_late_silique_2.vcf.recode.vcf > 505_filtered_common_late_silique_2.vcf.gz
# tabix -p vcf 505_filtered_common_leaf_2.vcf.gz
# tabix -p vcf 505_filtered_common_late_silique_2.vcf.gz

# import into R, using vcfR for manipulation 
library("vcfR")

late_silique_505_common <- read.vcfR("~/505/leaf_VS_late_silique/505_filtered_common_late_silique_2.vcf.gz")
leaf_505_common <- read.vcfR("~/505/leaf_VS_late_silique/505_filtered_common_leaf_2.vcf.gz")
...
geno.mds$tissue <- c(rep("leaf", 58), rep("late_silique", 58))

p.mds.country.2 <- ggplot(data=geno.mds, aes(V1, V2, color=factor(tissue)))
p.mds.country.2 <- p.mds.country.2 + geom_point(size=1) 
p.mds.country.2 <- p.mds.country.2 + scale_color_brewer(type="qual",palette="Set1")
# p.mds.country.2 <- p.mds.country.2 + geom_text_repel(aes(x=V1, y=V2, label=factor(Name)))
# p.mds.country <- p.mds.country + facet_wrap(~Sample.Description.Type)
...
p.mds.country.2 <- ggplot(data=geno.mds.all.des, aes(V1, V2, color=factor(tissue)))
p.mds.country.2 <- p.mds.country.2 + geom_point(size=1) 
p.mds.country.2 <- p.mds.country.2 + scale_color_brewer(type="qual",palette="Set1")
p.mds.country.2 <- p.mds.country.2 + geom_text_repel(aes(x=V1, y=V2, label=factor(Name)))
# p.mds.country <- p.mds.country + facet_wrap(~Sample.Description.Type)
...
p.dendro
ggsave("~/505/output/figure/clustering_common_leaf_late_silique.png", width = 15, height = 20)  
...
head(Erucic_acid)
SNP.Erucic <- data.frame(CHROM = gsub("(S)([[:print:]]+)(_)([[:digit:]]+)", "\\2", Erucic_acid[Erucic_acid$P.value < 0.00001,]$SNP), 
                         POS = gsub("(S)([[:print:]]+)(_)([[:digit:]]+)", "\\4", Erucic_acid[Erucic_acid$P.value < 0.00001,]$SNP))

SNP.Erucic$CHROM <- paste("chr", SNP.Erucic$CHROM, sep = "")
...
write.csv(SNP_gene_final_fxn, file = "~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/leaf/GWAS_SNP_gene_fxn.csv")

# as a summary, 24 out of the 28 SNPs fall within genes, 23 of them have arabidopsis homologs.
```
...
# 108, 109, and 88 phenotype data 
>>>>>>> c7ffd0e697012c8c12719f56a29fdaf86ba41434
```{r}
load("~/Desktop/Brassica_project/KIAT_RNA_seq/505/data/gh.seed.bolt.table.wide.Rdata")
gh.seed.bolt.table.wide
gh.seed.bolt.table.wide[gh.seed.bolt.table.wide$SN=="K108",]
gh.seed.bolt.table.wide[gh.seed.bolt.table.wide$SN=="K109",]
gh.seed.bolt.table.wide[gh.seed.bolt.table.wide$SN=="K88",]
# 
...
read.count <- read.table("~/505/data/bioftp.org/TBD170026_20170202/read.count.505.tsv", header=T, row.names=1)
head(read.count)
dim(read.count)
...
dim(read.count.small) # 53824    94  
save(read.count.small, file="~/505/output/leaf/read.count.small.Rdata")

# normalize 
dge.read.count <- DGEList(counts=read.count.small, group=read.count.small$group)
dge.read.count <- calcNormFactors(dge.read.count, method = "TMM")
hist(dge.read.count$samples$norm.factors)

# GLM model w/o reps 
design.read.count <- model.matrix(~0+group, data = read.count.sample)
bcv <- 0.4
dge.read.count <- estimateGLMCommonDisp(dge.read.count, design.read.count,verbose = T) # Disp = 0.13675 , BCV = 0.3698
...
distance_matrix$Row.names <- gsub("\\.","\\-", distance_matrix$Row.names) 
distance_matrix$Row.names <- gsub("Sample_", "", distance_matrix$Row.names)
...
distance_matrix.all <- merge(distance_matrix, sample_des_revised, by = "Sample.ID")

library(ggrepel)
set.seed(111) 

p.mds.country <- ggplot(data=distance_matrix.all, aes(mds$x, mds$y, color=factor(Sample.Description.Origin)))
p.mds.country <- p.mds.country + geom_point(size=1) 
p.mds.country <- p.mds.country + scale_color_brewer(type="qual",palette="Set1")
p.mds.country <- p.mds.country + geom_text_repel(aes(x=mds$x, y=mds$y, label=factor(Name)))
# p.mds.country <- p.mds.country + facet_wrap(~Sample.Description.Type)
p.mds.country 
ggsave(p.mds.country, filename="~/505/output/figure/p.mds.country.expression.png", height=20, width=20)
...
lrt.read.count <- glmLRT(fit.read.count, coef=2:94) # find any genes that are differentially expressed compared to the reference genotype (the intercept) 
topTags(lrt.read.count)
DEgene.505 <- topTags(lrt.read.count,n = Inf)$table[topTags(lrt.read.count,n = Inf)$table$FDR<0.001,]
dim(DEgene.505) # 53824 
``` 
...
# biocLite("DESeq2")
library("DESeq2")  
load("~/505/output/leaf/read.count.small.Rdata")

dds.505.leaf <- DESeqDataSetFromMatrix(countData = round(read.count.small), colData = read.count.sample, design = ~0+group)

vsd.505.leaf <- varianceStabilizingTransformation(dds.505.leaf)
vstMat.505.leaf <- assay(vsd.505.leaf)
colnames(vstMat.505.leaf) <- colnames(read.count.small)
# save(vstMat.505.leaf, file = "~/505/output/leaf/vstMat.505.leaf.Rdata")
load("~/505/output/leaf/vstMat.505.leaf.Rdata")

colnames(vstMat.505.leaf) <- gsub("Sample_", "", colnames(vstMat.505.leaf))
colnames(vstMat.505.leaf) <- gsub("\\.", "\\-", colnames(vstMat.505.leaf))
colnames(vstMat.505.leaf)

## pull out the vst transformed expression value for the 20 unique genes 
SNP.gene.fxn <-read.csv("~/505/output/leaf/GWAS_SNP_gene_fxn.csv", header=T)
vstMat.20gene <- as.data.frame(vstMat.505.leaf[which(rownames(vstMat.505.leaf) %in% SNP.gene.fxn$gene_ID),])
dim(vstMat.20gene) # 20 94 
head(vstMat.20gene)

# get the phenotypic trait values 
load("~/505/data/phenotype/gh.seed.bolt.table.wide.Rdata")
sample_des_revised.leaf
gh.seed.bolt.table.wide
...
gh.seed.bolt.table.wide.merge$Sample.ID

colnames(gh.seed.bolt.table.wide.merge) <- c("SN", "Sample.ID", "No.of.sowing", "name", "bolting", "Erucic_acid", "Oil_content", "Oleic_acid", "flowing", "germination_rate", "No.of.germination.seeds", "No.of.sowing.seeds")

head(gh.seed.bolt.table.wide.merge)
vstMat.20gene.t <- data.frame(t(vstMat.20gene))
vstMat.20gene.t$Sample.ID <- rownames(vstMat.20gene.t) 
vstMat.20gene.t

trait.gene <- merge(vstMat.20gene.t, gh.seed.bolt.table.wide.merge, by="Sample.ID")
dim(trait.gene) # 94 32 
summary(trait.gene)
trait.gene.sub <- trait.gene[!is.na(trait.gene$Erucic_acid),]
dim(trait.gene.sub)
summary(trait.gene.sub) 

colnames(trait.gene)

# finally get to cor.test() part 
# 1) get pearson correlation test result 
pearson.estimate <- data.frame()

for (i in 1:3){
  for (j in 1:20){
    pearson.estimate[i,j] <- cor(trait.gene.sub[,1+j], trait.gene.sub[,25+i])
  }
}

pearson.estimate
dim(pearson.estimate) # 3 20 
rownames(pearson.estimate) <- colnames(trait.gene.sub)[26:28]
colnames(pearson.estimate) <- colnames(trait.gene.sub)[2:21] 

# how to get p-value all together 
pearson.pvalue <- data.frame()
tmp <- data.frame()

for (i in 1:3){
  for (j in 1:20){
    tmp <- cor.test(trait.gene.sub[,1+j], trait.gene.sub[,25+i])
    pearson.pvalue[i,j] <- tmp$p.value
  }
}
...
=======
pearson.pvalue
dim(pearson.pvalue) # 3 20 
rownames(pearson.pvalue) <- colnames(trait.gene.sub)[26:28]
colnames(pearson.pvalue) <- colnames(trait.gene.sub)[2:21] 

# reform for ggplot 
pearson.estimate$trait <- rownames(pearson.estimate)
pearson.estimate
pearson.result <- melt(pearson.estimate, id.vars="trait") 
pearson.result$pvalue <- as.data.frame(melt(pearson.pvalue))[,"value"]
pearson.result$sig <- ifelse(pearson.result$pvalue < 0.05, "*", " ")
pearson.result
save(pearson.result, file="~/505/output/pearson.reult.Rdata")

# 2) get spearman corrlelation test result 
spearman.estimate <- data.frame()

for (i in 1:3){
  for (j in 1:20){
    spearman.estimate[i,j] <- cor(trait.gene.sub[,1+j], trait.gene.sub[,25+i], method="spearman")
  }
}

spearman.estimate
dim(spearman.estimate) # 3 20 
rownames(spearman.estimate) <- colnames(trait.gene.sub)[26:28]
colnames(spearman.estimate) <- colnames(trait.gene.sub)[2:21] 
...
for (i in 1:3){
  for (j in 1:20){
    tmp <- cor.test(trait.gene.sub[,1+j], trait.gene.sub[,25+i], method="spearman")
    spearman.pvalue[i,j] <- tmp$p.value
  }
}

spearman.pvalue
dim(spearman.pvalue) # 3 20 
rownames(spearman.pvalue) <- colnames(trait.gene.sub)[26:28]
colnames(spearman.pvalue) <- colnames(trait.gene.sub)[2:21] 
...
spearman.result
save(spearman.result, file="~/505/output/spearman.reult.Rdata")

# 3) get figure out 
library(ggplot2)

head(pearson.result)
p.pearson <- ggplot(data=pearson.result) 
p.pearson <- p.pearson + geom_col(aes(x=variable, y=value))
p.pearson <- p.pearson + geom_text(aes(x=variable, y=value, label=sig), col="red", size=5, position=position_dodge(width=1))
p.pearson <- p.pearson + facet_grid(trait ~ variable)
p.pearson <- p.pearson + geom_hline(yintercept=0)
p.pearson <- p.pearson + labs(x="", y="pearson corrlelation value") 
p.pearson 

ggsave(p.pearson, filename="~/505/output/figure/p.pearson.png", width=22, height=7)  

head(pearson.result)
p.spearman <- ggplot(data=spearman.result) 
p.spearman <- p.spearman + geom_col(aes(x=variable, y=value))
p.spearman <- p.spearman + geom_text(aes(x=variable, y=value, label=sig), col="red", size=5, position=position_dodge(width=1))
p.spearman <- p.spearman + facet_grid(trait ~ variable)
p.spearman <- p.spearman + geom_hline(yintercept=0)
p.spearman <- p.spearman + labs(x="", y="spearman corrlelation value") 
p.spearman 

ggsave(p.spearman, filename="~/505/output/figure/p.spearman.png", width=22, height=7)
>>>>>>> 3c6d53514e89c8ceb44298a94bd32e1e164469fe
```

# SNP annotation for significant SNPs 
```{r} 
annotation.28SNP <- read.table("~/505/vcf_leaf_no_pop/combined/missing_0.5/sinigicant_SNP_ann", header=F) 
dim(annotation.28SNP)

annotation.28SNP.sub <- annotation.28SNP[,c(1:8)] 
annotation.28SNP.sub
filter(annotation.28SNP, V3=="SC09_19073712") %>% select(V8) 
```

```{r}
load("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/pearson.reult.Rdata")
pearson.result
p.pearson <- ggplot(data = pearson.result)
