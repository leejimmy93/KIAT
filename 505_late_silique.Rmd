...
p.total.reads <- p.total.reads + theme(axis.text.x = element_text(angle = 90, size = 8)) 
p.total.reads   
# ggsave(p.total.reads, filename = "~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/figure/total_reads_CDE.png", height = 8, width = 11) 
...
pl.mapping.indi.e <- pl.mapping.indi.e + labs(list(title = "", x = "individual sample", y = "reads number")) + theme(axis.text.x=element_text(angle = 90, size = 8)) + ylim(0, 4.5e+7)
pl.mapping.indi.e 
...
pl.mapping.indi.final 
# save_plot("~/Desktop/Brassica_project/KIAT_RNA_seq/505/output/figure/pl_mapping_indi_CDE.png", pl.mapping.indi.final, base_aspect_ratio = 1, base_width = 12)  
...
# use common.lines for GWAS in both leaf & late silique sample 
```{r}
load("~/505/output/common.lines.leaf.131.sample.Rdata")
# write.table(common.lines, file="~/505/leaf_VS_late_silique/common.lines.92.txt")
# cat common.lines.92.txt | awk 'BEGIN{FS="\""}{print $8}' > late_silique_common_ID_92
# cat common.lines.92.txt | awk 'BEGIN{FS="\""}{print $6}' | grep -v "Taxa" > leaf_common_ID_92
# 
# vcftools --keep leaf_common_ID_92 --gzvcf ../vcf_leaf_no_pop/combined/missing_0.5/505_filtered.vcf.gz --recode --recode-INFO-all --out 505_filtered_common_leaf_92 
```

# plot out phenotype data 
```{r}
load("~/Desktop/Brassica_project/KIAT_RNA_seq/505/data/pheno.505.Rdata")
pheno_data_505
hist(pheno_data_505$Erucic_acid)
hist(pheno_data_505$Oleic_acid) 
```
