...
tmp <- (merge(parent.read.count.one.small, fatty_acid_gene_index, by="row.names"))[,1:28]
head(tmp)
...
p.fatty.acid <- p.fatty.acid + facet_grid(gt~geneID)
# p.fatty.acid <- p.fatty.acid + geom_errorbar(mapping=aes(x=tissue,ymin=min,ymax=max))
...
sapply(colnames(parent.read.count.one.small), function(RNAlib){
  sum(parent.read.count.one.small[,RNAlib]>10)
}
)
...
sapply(colnames((parent.read.count.one.small)), function(RNAlib){
  rownames(head(parent.read.count.one.small[order(parent.read.count.one.small[,RNAlib],decreasing = TRUE),], 10))
}
)
...
temp <- sample_des[,c(4,5,6,10)]
rownames(temp) <- paste(sample_des[,4], sample_des[,5], sample_des[,6], sep = "_")
sample_statistics <- merge(temp, expressed_gene_number, by=0) 
colnames(sample_statistics) <- c("sample_ID", "Cultivar", "Tissue", "rep", "Total_Bases_Gb", "expressed_gene_number")
...
# start ggplot here 
p3 <- ggplot(data = segment(ggdata.parent))
p3 <- p3 + geom_segment(aes(x=x, y=y, xend=xend, yend=yend)) + theme_dendro()
p3 <- p3 + geom_text(data = label(ggdata.parent), aes(x = x, y = y, label = label, hjust=0, color=group)) + coord_flip() + scale_y_reverse(expand=c(0.2, 0))
p3
