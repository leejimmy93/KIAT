...
```{r}
# mapping result
mapping_result <- read.table("~/2016_summer/raw_data/Star_Stats.tab", header = T)
head(mapping_result)
mapping_result$Sample.ID <- gsub("([[:print:]]+)(_)(paired.star.trim.dir)", "\\1", mapping_result$Sample) 
head(mapping_result)
tail(mapping_result)
...
library(ggplot2) 
p.total.reads <- ggplot(data=mapping_result)
p.total.reads <- p.total.reads + geom_bar(aes(x=reorder(Sample.ID, Number_Input_Reads), y=Number_Input_Reads, fill=batch), stat = "identity")
...
ggsave(p.length.distr, filename = "~/Desktop/Brassica_project/KIAT_RNA_seq/assembly/p.length.dist.png", width = 12, height=8) 
