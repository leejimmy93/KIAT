...
### index bam file 
# samtools index Ae_no_bolting_rmdup.bam (screen -r 35910.ttys009.coloma) 
# samtools index Ol_no_bolting_rmdup.bam (screen -r 35901.ttys009.coloma) 
...
dim(parent.read.count.2.small) # 57925    24   
...
distance_matrix.2$group <- gsub("(Da-Ae|Da-Ol-1)(_)(Young|flowering|early-silique|late-silique)(_)(1|2|3)","\\1\\3",distance_matrix.2$Row.names) 
distance_matrix.2$gt <- gsub("(Da-Ae|Da-Ol-1)(_)(Young|flowering|early-silique|late-silique)(_)(1|2|3)","\\1",distance_matrix.2$Row.names)
distance_matrix.2$tissue <- gsub("(Da-Ae|Da-Ol-1)(_)(Young|flowering|early-silique|late-silique)(_)(1|2|3)","\\3",distance_matrix.2$Row.names)

colnames(distance_matrix.2) <- c("lib","x","y","group","gt","tissue")
head(distance_matrix.2)

# making color MDS figure 
p.mds.2 <- ggplot(data = distance_matrix.2)
p.mds.2 <- p.mds.2 + geom_point(aes(x, y, color=factor(tissue)), size=3) 
p.mds.2 <- p.mds.2 + labs(y = "BCV distance 2", x="BCV distance 1")
p.mds.2 <- p.mds.2 + facet_grid(~gt)
p.mds.2     

# ggsave("/Users/ruijuanli/Desktop/Brassica_project/KIAT_RNA_seq/figure/genome_mapped/MDS.png", width = 11, height = 8)
...
p.dendro.2
# ggsave("/Users/ruijuanli/Desktop/Brassica_project/KIAT_RNA_seq/figure/genome_mapped/clustering_new.png", width = 15, height = 8) 
```
...
# w/o batch effect
dge.data.parent.2 <- estimateGLMCommonDisp(dge.data.parent.2, design.parent.2,verbose = TRUE) # Disp = 0.27855 , BCV = 0.5278
...
lrt.young.2 <- glmLRT(fit.parent.2, contrast = c(0,0,0,1,0,0,0,-1))
topTags(lrt.young.2)
...
lrt.flowering.2 <- glmLRT(fit.parent.2, contrast = c(0,1,0,0,0,-1,0,0))
...
lrt.early.silique.2 <- glmLRT(fit.parent.2, contrast = c(1,0,0,0,-1,0,0,0))
...
lrt.late.silique.2 <- glmLRT(fit.parent.2, contrast = c(0,0,1,0,0,0,-1,0))
...
DEGs_number_between_gt.melt.parent.2$DE <- rep(c("down", "up"), 4)
...
### making ggplot for DEGs 
p.DEGs_number_between_gt.parent.2 <- ggplot(data = DEGs_number_between_gt.melt.parent.2)
...
## between developmental stages 
# Da-Ae 
lrt.young.vs.flowering.ae.2 <- glmLRT(fit.parent.2, contrast = c(0,1,0,-1,0,0,0,0))
topTags(lrt.young.vs.flowering.ae.2)
summary(de.young.vs.flowering.ae.2 <- decideTestsDGE(lrt.young.vs.flowering.ae.2, p=0.05))
DEgene.young.vs.flowering.ae.2 <- topTags(lrt.young.vs.flowering.ae.2,n = Inf)$table[topTags(lrt.young.vs.flowering.ae.2,n = Inf)$table$FDR<0.05,]
# -1   442
# 0  59872
# 1    661
############ stopped here... 
