...
napus_vs_ara <- read.delim("~/Desktop/Brassica_project/reference/B.napus_vs_ara.final")
...
fatty.acid.early.silique.data <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/fatty_acid_related/table1.csv")
fatty.acid.late.silique.data <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/fatty_acid_related/table2.csv")
fatty.acid.Ae.data <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/fatty_acid_related/table3.csv") 
fatty.acid.Ol.data <- read.csv("~/Desktop/Brassica_project/KIAT_RNA_seq/fatty_acid_related/table4.csv")

head(napus_vs_ara.final)

fatty.acid.early.silique <- data.frame(napus_ID = fatty.acid.early.silique.data$geneID,
                                       early_silique = fatty.acid.early.silique.data$logFC)
fatty.acid.late.silique <- data.frame(napus_ID = fatty.acid.late.silique.data$geneID,
                                       late_silique = fatty.acid.late.silique.data$logFC)
fatty.acid.Ae <- data.frame(napus_ID = fatty.acid.Ae.data$geneID,
                                       Ae = fatty.acid.Ae$logFC)
fatty.acid.Ol <- data.frame(napus_ID = fatty.acid.Ol.data$geneID,
                                       Ol = fatty.acid.Ol.data$logFC)

tmp <- merge(fatty.acid.early.silique, fatty.acid.late.silique, by="napus_ID", all=T)
tmp <- merge(tmp, fatty.acid.Ae, by="napus_ID", all=T)
tmp <- merge(tmp, fatty.acid.Ol, by="napus_ID", all=T)

tmp <- merge(tmp, napus_vs_ara.final, by="napus_ID")[, c(6,2,3,4,5)]
tmp
write.csv(tmp, file = "~/Desktop/tmp.csv")
