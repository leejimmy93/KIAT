...
nrow(tmp[rowSums(tmp > 3000),])
five.fatty.acid.genes <- head(tmp[order(rowSums(tmp > 3000), decreasing = T),], 5)
...
# draw plot 
tmp3 <- read.csv("/Users/ruijuanli/Desktop/5_fatty_acid_genes.csv")
gene1 <- tmp3[1:5,]
gene2 <- tmp3[11:20,]
gene3 <- tmp3[21:30,]
gene4 <- tmp3[31:40,]
...
ggplot(gene1, aes(x=tt, y=mean), geom_line())
```
...
set.seed(123)
fit <- pvclust(vstMat.parent, method.hclust = "ward.D", method.dist = "euclidean", nboot = 1000) # to generate the report
