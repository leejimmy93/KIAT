...
vcf.Ae.Ol.no.bolting.unique <- read.table("~/Desktop/Brassica_project/KIAT_RNA_seq/SNP_calling/data/Ae_Ol_unique_modified.vcf",as.is=T,na.strings = ".")
head(vcf.Ae.Ol.no.bolting.unique) 

vcf.header.Ae.Ol.unique <- system("grep '#C' ~/Desktop/Brassica_project/KIAT_RNA_seq/SNP_calling/data/Ae_Ol_unique_modified.vcf",intern = TRUE) 
...
system("grep '##INFO' ~/Desktop/Brassica_project/KIAT_RNA_seq/SNP_calling/data/Ae_Ol_unique_modified.vcf")
system("grep '##FORMAT' ~/Desktop/Brassica_project/KIAT_RNA_seq/SNP_calling/data/Ae_Ol_unique_modified.vcf") 
...
# alter allele count > 2 & not equal to NA 
Ae_ref_after_filter.unique <- Ae_ref.unique[Ae_ref.unique$Ae_alt.depth > 2 & is.na(Ae_ref.unique$Ae_alt.depth)=="FALSE",] 
dim(Ae_ref_after_filter.unique) # 403691     23  
...
# alter allele count > 2 & not equal to NA 
Ol_ref_after_filter.unique <- Ol_ref.unique[Ol_ref.unique$Ol_alt.depth > 2 & is.na(Ol_ref.unique$Ol_alt.depth)=="FALSE",] 
dim(Ol_ref_after_filter.unique) # 397122     23  
...
# alter allele count > 2 & not equal to NA
sum(vcf.Ae.Ol.no.bolting.unique.HQ.new[vcf.Ae.Ol.no.bolting.unique.HQ.new$Ae_gt=="1/1/1/1",]$Ae_alt.depth > 2) # 140950

Ae_Ol_after_filter.unique  <- 
vcf.Ae.Ol.no.bolting.unique.HQ.new[(vcf.Ae.Ol.no.bolting.unique.HQ.new$Ae_gt=="1/1/1/1" & vcf.Ae.Ol.no.bolting.unique.HQ.new$Ae_alt.depth > 2) | (vcf.Ae.Ol.no.bolting.unique.HQ.new$Ol_gt=="1/1/1/1" & vcf.Ae.Ol.no.bolting.unique.HQ.new$Ol_alt.depth > 2),] 
dim(Ae_Ol_after_filter.unique)  # 271105  
  
# write(vcf.Ae.Ol.no.bolting.HQ.new, file = "~/Desktop/tmp.txt") 

######### using IGV to check SNP 
# vcf.Ae.Ol.no.bolting.unique.HQ.new[(vcf.Ae.Ol.no.bolting.unique.HQ.new$Ae_gt == "1/1/1/1"),] 
# ...  
...
# do a little calculation: how many SNPs on main & random chromosomes? 
sum(SNP_chr_main.unique$V1) # 232395  
sum(SNP_chr_random.unique$V1) # 38710  
235214/275083 # 85.5%
...
dim(Ae_Ol_after_filter.main.unique) # 232395     23  
...
# number of SNPs on A & C subgenome
sum(Ae_Ol_after_filter.main.unique$subgenome=="A")/nrow(Ae_Ol_after_filter.main.unique) # 68%
...
dim(Ae_Ol_after_filter.random.unique) # 38710    23  
...
# do a little calculation: how many SNPs on main & random chromosomes?
sum(SNP_chr_main_Ae_ref.unique$number) # 333713
sum(SNP_chr_random_Ae_ref.unique$number) # 69978
235214/275083 # 85.5%

sum(SNP_chr_main_Ae_ref.unique[SNP_chr_main_Ae_ref.unique$subgenome=="A",]$number_Ae_ref) # 209113 
sum(SNP_chr_main_Ae_ref.unique[SNP_chr_main_Ae_ref.unique$subgenome=="C",]$number_Ae_ref) # 124600  
...
# do a little calculation: how many SNPs on main & random chromosomes?
sum(SNP_chr_main_Ol_ref.unique$number) # 326149 
sum(SNP_chr_random_Ol_ref.unique$number) # 70973 

sum(SNP_chr_main_Ol_ref.unique[SNP_chr_main_Ol_ref.unique$subgenome=="A",]$number_Ol_ref) # 191590 
sum(SNP_chr_main_Ol_ref.unique[SNP_chr_main_Ol_ref.unique$subgenome=="C",]$number_Ol_ref) # 134559
...
# 4) Variant calling 
# ran with chromosome splitted (use script GATK.sh)    
...
# 5) variant filtering (something wrong...come back later) 
# GATK -T VariantFiltration -R ~/Reference/B.napus/Brassica_napus_v4.1.chromosomes.fa -V Ae_ref.vcf -window 35 -cluster 3 -filterName FS -filter "FS > 30.0" -filterName QD -filter "QD < 2.0" -o Ae_ref_filtered.vcf
# GATK -T VariantFiltration -R ~/Reference/B.napus/Brassica_napus_v4.1.chromosomes.fa -V Ol_ref.vcf -window 35 -cluster 3 -filterName FS -filter "FS > 30.0" -filterName QD -filter "QD < 2.0" -o Ol_ref_filtered.vcf 
...
# additional filter  
```{r}
### visulize unique & non-unique SNPs in IGV 
# export Ae VS Ol SNP dataset 
head(Ae_Ol_after_filter.main.unique)

colnames(Ae_Ol_after_filter.main)
dim(Ae_Ol_after_filter.main.unique); dim(Ae_Ol_after_filter.main)  

Ae_Ol_after_filter.main.unique.export <- Ae_Ol_after_filter.main.unique[,c(1:11)]
Ae_Ol_after_filter.main.export <- Ae_Ol_after_filter.main[,c(1:11)]
head(Ae_Ol_after_filter.main.unique.export)


write.matrix()
write.table(Ae_Ol_after_filter.main.unique.export, file = "~/Desktop/Brassica_project/KIAT_RNA_seq/SNP_calling/output/Ae_Ol_after_filter.main.unique.export.txt", row.names=FALSE)
