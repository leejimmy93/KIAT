...
# also check the ref/alt allele ratio for the het calls, distribution plot for Ae & Ol 
vcf.Ae.Ol.no.bolting.unique.HQ.Ol.het <- vcf.Ae.Ol.no.bolting.unique.HQ[(vcf.Ae.Ol.no.bolting.unique.HQ$Ol_gt=="0/1"),]

p.Ol.het <- ggplot(data = vcf.Ae.Ol.no.bolting.unique.HQ.Ol.het) 
p.Ol.het <- p.Ol.het + geom_histogram(aes(Ol_alt.depth/Ol_ref.depth)) + scale_x_log10() 
p.Ol.het <- p.Ol.het + labs(list(x="log10(Ol_alt.depth/Ol_ref.depth)"))
  
pl.SNP.main.1.unique <- pl.SNP.main.1.unique + labs(list(title = "", x = "chromosome ID", y = "number of SNPs"))
  
p.Ol.het
ggsave(p.Ol.het, filename = "~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/p.Ol.het.png", height = 8, width = 11)

vcf.Ae.Ol.no.bolting.unique.HQ.Ol.homo.alt <- vcf.Ae.Ol.no.bolting.unique.HQ[(vcf.Ae.Ol.no.bolting.unique.HQ$Ol_gt=="1/1"),]


vcf.Ae.Ol.no.bolting.unique.HQ.Ol.homo.alt[vcf.Ae.Ol.no.bolting.unique.HQ.Ol.homo.alt$Ol_ref.depth==4128,] 


vcf.Ae.Ol.no.bolting.unique.HQ.Ae.het <- vcf.Ae.Ol.no.bolting.unique.HQ[(vcf.Ae.Ol.no.bolting.unique.HQ$Ae_gt=="0/1"),]

p.Ae.het <- ggplot(data = vcf.Ae.Ol.no.bolting.unique.HQ.Ae.het) 
p.Ae.het <- p.Ae.het + geom_histogram(aes(Ae_alt.depth/Ae_ref.depth)) + scale_x_log10() 
p.Ae.het <- p.Ae.het +  
p.Ae.het
