...
###### data from non-biallielic 
```{r}
source("/Users/ruijuanli/Desktop/Brassica_project/KIAT_RNA_seq/analysis/function_BnRNAseq.R")
vcf.freebayes.non.biallelic <- read.table("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/freebayes/as_diploid/no_bolting_unique_mapped/Ae_Ol_modified.vcf",as.is=T,na.strings = ".")  
dim(vcf.freebayes.non.biallelic) # 2053680      11
vcf.freebayes.non.biallelic$V11 

# get header 
vcf.header.freebayes <- system("grep '#C' ~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/freebayes/as_diploid/no_bolting_unique_mapped/Ae_Ol_modified.vcf",intern = TRUE) 
...
vcf.freebayes.non.biallelic.reform <- SNP.freebayes.reformat(vcf.freebayes.non.biallelic, vcf.header.freebayes) 
dim(vcf.freebayes.non.biallelic.reform) # 2053680      25  
table(vcf.freebayes.non.biallelic.reform$Ae_gt)
#  0/0    0/1    0/2    0/3    0/4    1/1    1/2    1/3    1/4    2/2    2/3    2/4    3/3    3/4     NA 
# 502869 627633   6202     49      3 648030  18445    500      6   5202    115      2    150      6 244468 

table(vcf.freebayes.non.biallelic.reform$Ol_gt)
#  0/0    0/1    0/2    0/3    0/4    1/1    1/2    1/3    1/4    2/2    2/3    2/4    3/3    3/4    4/4 
# 546937 611663   6275     63      1 640782  17448    523      4   4504     98      2    144      5      1 
#    4/5     NA 
#      1 225229 
# save(vcf.freebayes.non.biallelic.reform, file = "~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/output/vcf.freebayes.non.biallelic.reform.Rdata")

vcf.freebayes.non.biallelic.reform.Ae.het <- vcf.freebayes.non.biallelic.reform[vcf.freebayes.non.biallelic.reform$Ae_gt=="0/1",]
sum(vcf.freebayes.non.biallelic.reform.Ae.het$Ae_alt.depth==0, na.rm = T)
sum(vcf.freebayes.non.biallelic.reform.Ae.het$Ae_ref.depth==0, na.rm = T) 

head(vcf.freebayes.non.biallelic.reform.Ae.het)
head(vcf.freebayes.non.biallelic.reform.Ae.het[which(vcf.freebayes.non.biallelic.reform.Ae.het$Ae_alt.depth==0),], 100)
...
```{r}
source("/Users/ruijuanli/Desktop/Brassica_project/KIAT_RNA_seq/analysis/function_BnRNAseq.R")
vcf.freebayes <- read.table("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/freebayes/as_diploid/no_bolting_unique_mapped/Ae_Ol_modified_SNPonly_biallelic.recode.vcf",as.is=T,na.strings = ".:.:.:.:.:.:.") 
...
vcf.freebayes.reform.Ae.het <- vcf.freebayes.reform[vcf.freebayes.reform$Ae_gt=="0/1",]
sum(vcf.freebayes.reform.Ae.het$Ae_alt.depth==0)
sum(vcf.freebayes.reform.Ae.het$Ae_ref.depth==0) 

# save(vcf.freebayes.reform, file = "~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/output/vcf.freebayes.reform.Rdata")
save(vcf.freebayes.reform.Ae.het, file = "~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/output/vcf.freebayes.reform.Ae.het.Rdata")

```

# calculate genotype quality as what we get from GATK... 
```{r}
load("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/output/vcf.freebayes.reform.Rdata")
head(vcf.freebayes.reform)
...
#########
load("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/output/vcf.freebayes.reform.Rdata")
load("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/output/vcf.freebayes.reform.Ae.het.Rdata")
...
# check why there are so many heterozygous
```{r}
...
vcf.freebayes.HQ$Ae_gt=="1/1" 
vcf.freebayes.HQ$Ae_gt=="0/1"
vcf.freebayes.HQ$Ae_gt=="0/1" 
...
vcf.freebayes.HQ.1.Ae.het <- vcf.freebayes.HQ.1[(vcf.freebayes.HQ.1$Ae_gt=="0/1"),]
sum(vcf.freebayes.HQ.1.Ae.het$Ae_ref.depth=="0")
sum(vcf.freebayes.HQ.1.Ae.het$Ae_alt.depth=="0") # great that no 0 anymore... 

vcf.freebayes.HQ.1.Ol.het <- vcf.freebayes.HQ.1[(vcf.freebayes.HQ.1$Ol_gt=="0/1"),]
sum(vcf.freebayes.HQ.1.Ol.het$Ol_ref.depth=="0")
sum(vcf.freebayes.HQ.1.Ol.het$Ol_alt.depth=="0") # great that no 0 anymore... 
...
load("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/output/vcf.freebayes.HQ.1.Ae.het.tmp.chrA01.Rdata")
...
cor.test(vcf.freebayes.HQ.1.Ae.het.tmp.chrA01$Name, vcf.freebayes.HQ.1.Ae.het.tmp.chrA01$allele_ratio)





# check the corrleation between gene name & allele ratio 
# library(brms)
# library(rstan)
# 
...
# 
# vcf.freebayes.HQ.1.Ae.het.tmp.chrA01$allele.ratio <- vcf.freebayes.HQ.1.Ae.het.tmp.chrA01$Ae_alt.depth/vcf.freebayes.HQ.1.Ae.het.tmp.chrA01$Ae_ref.depth
# 
# m.allele.ratio <- brm(allele.ratio ~ as.factor(Name),
#                       data = vcf.freebayes.HQ.1.Ae.het.tmp.chrA01, 
#                       prior = set_prior("normal(0, 10)", class = "b")
#                       )
# 
# summary(m.allele.ratio)

# visually check whether there is association between gene and allele freqeuncy 
# pick ratio == 0.1 
vcf.freebayes.HQ.1.Ae.het.tmp.chrA01.0.5 <- vcf.freebayes.HQ.1.Ae.het.tmp.chrA01[vcf.freebayes.HQ.1.Ae.het.tmp.chrA01$allele.ratio <"0.5",]
View(vcf.freebayes.HQ.1.Ae.het.tmp.chrA01.0.1)

# pick ratio == 10 
vcf.freebayes.HQ.1.Ae.het.tmp.chrA01.5 <- vcf.freebayes.HQ.1.Ae.het.tmp.chrA01[vcf.freebayes.HQ.1.Ae.het.tmp.chrA01$allele.ratio >"5",]
View(vcf.freebayes.HQ.1.Ae.het.tmp.chrA01.5)
...
# check CIGAR score: corrlelation between CIGAR score... 
...
```{r}
# 1) prepare for SNP calling on whitney using 
# SNP_calling_GATK_pipeline.sh (https://github.com/leejimmy93/KIAT_whitney/blob/master/parent_SNP/SNP_calling_GATK_pipeline.sh)

# 2) SNP calling on cabernet 
# SNP_calling_array_no_bolting.slurm (https://github.com/leejimmy93/KIAT_cabernet/tree/master/parent_SNP)

# 3) combine vcf from different chromosomes 
# GATK_combine.sh (https://github.com/leejimmy93/KIAT_whitney/blob/master/parent_SNP/GATK_combine.sh)

# 4) VCF format check/validate 
...
# data location: /Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/SNP_parent/result/GATK/raw_vcf/vcf_split_by_chr/ 

# 5) extract only biallelic SNPs from vcf files 
# extract_SNP.sh & extract_biallelic.sh (https://github.com/leejimmy93/KIAT_whitney/tree/master/parent_SNP)

# 6) import into R for analysis 


######## still need to get data from chr_unn 
```

# SNP filtering from GATK (filtered)  
```{r} 
...
vcf.GATK.A01.biallelic <- read.table("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/GATK/as_diploid/no_bolting/xaa.intervals.SNP.biallelic.recode.vcf",as.is=T,na.strings = ".") 

head(vcf.GATK.A01.biallelic, 100) 
vcf.header.GATK <- system("grep '#C' ~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/GATK/as_diploid/no_bolting/xab.intervals.vcf.filtered.SNP.biallelic.recode.vcf",intern = TRUE) 
vcf.header.GATK <- sub("#","",vcf.header.GATK) #get rid of the pound sign
vcf.header.GATK <- unlist(strsplit(vcf.header.GATK,split="\t"))

vcf.GATK.reform <- SNP.GATK.reformat(vcf.GATK.A01.biallelic, vcf.header.GATK)
dim(vcf.GATK.reform) # 67703    21 
head(vcf.GATK.reform)
vcf.GATK.reform$Ae_gt

vcf.GATK.reform[which(vcf.GATK.reform$Ol_gt=="NA"),]
...
dim(vcf.GATK.HQ) # 29850    21 

``` 

# 2nd round of filtering 
```{r}
### how to exclude NA records from the file... 
# 1) max read depth, should not be much higher than average read depth 
mean(vcf.GATK.HQ$Ae_approx.depth, na.rm=T)
mean(vcf.GATK.HQ$Ol_approx.depth, na.rm=T)
# use 100 as the max 
...
vcf.GATK.HQ.1 <- vcf.GATK.HQ[(!is.na(vcf.GATK.HQ$Ae_approx.depth) & 
                              ...
                              vcf.GATK.HQ$Ae_approx.depth < 120 & 
                              vcf.GATK.HQ$Ol_approx.depth < 120),]
dim(vcf.GATK.HQ.1) # 18823    21
head(vcf.GATK.HQ.1) 

# 2) genotype quality, greater than 60 maybe  
hist(vcf.GATK.HQ.1$Ae_genotype.qual)
...
vcf.GATK.HQ.2 <- vcf.GATK.HQ.1[(!is.na(vcf.GATK.HQ.1$Ae_genotype.qual) &
                                ...
                                vcf.GATK.HQ.1$Ae_genotype.qual > 60 & 
                                vcf.GATK.HQ.1$Ol_genotype.qual > 60),]

dim(vcf.GATK.HQ.2)
table(vcf.GATK.HQ.2$Ae_gt)
table(vcf.GATK.HQ.2$Ol_gt)
vcf.GATK.HQ.2.Ae.het$Ae_ref.alt.depth

# 3) based on allele balance, an arbitrary number 
...
vcf.GATK.HQ.2.Ae.het$Ae_alt.depth/vcf.GATK.HQ.2.Ae.het$Ae_ref.depth
vcf.GATK.HQ.2.Ae.het
...
pl.allele.ratio.Ol(vcf.GATK.reform.HQ.filtered.Ol.het)   
...
# import into R for late analysis  
