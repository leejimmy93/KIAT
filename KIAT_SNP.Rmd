...
dim(ANN_gene_ID) # 492993      1 ###### why??? 



# get gene name for each SNP
head(strsplit(vcf.freebayes.HQ.1$ANN,split = "|"))

vcf.freebayes.HQ.1$tmp <-  gsub("([[:print:]]+)(,)([[:print:]])", "\\1", vcf.freebayes.HQ.1$ANN) 

head(vcf.freebayes.HQ.1$tmp, 100)
dim(vcf.freebayes.HQ.1)

vcf.freebayes.HQ.1$gene_ID <- gsub("(A|T|C|G)(|)([[:print:]]+)(|)(MODIFIER|MODERATE|HIGH|LOW)(|)([[:print:]]+)(|)([[:print:]])", "\\7", vcf.freebayes.HQ.1$tmp) 

# vcf.freebayes.HQ.1$tmp <- gsub("(A|T|C|G)(|)([[:print:]]+)(|)(MODIFIER|MODERATE|HIGH|LOW)(|)([[:print:]]+)(|)([[:print:]]+)(|)(intergenic_region|transcript)(|)([[:print:]])", "\\1", vcf.freebayes.HQ.1$ANN) 

tail(vcf.freebayes.HQ.1$gene_ID, 100)
...
# 2) genotype quality, greater than 30 maybe  
...
####### gene correlation with MAF
# need to reformat data to disset INFO column 
head(vcf.GATK.HQ.2) 
info <- matrix(
  unlist(strsplit(vcf.GATK.HQ.2$INFO,split = ";")),
  nrow=nrow(vcf.GATK.HQ.2),  
  byrow=TRUE
  ) 

head(info)
head(unlist(strsplit(vcf.GATK.HQ.2$INFO,split = ";")), 100)
length(unlist(strsplit(vcf.GATK.HQ.2$INFO,split = ";")))/nrow(vcf.GATK.HQ.2)
