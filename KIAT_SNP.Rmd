...
# this way doesn't work because then I get very few 0/0.... but still this can be used to understand RPL & RLR... 
...
dim(vcf.freebayes.HQ.Ae) # 1174345      25  
nrow(vcf.freebayes.HQ.Ae) / nrow(vcf.freebayes.reform.Ae) # 70% of SNPs were retained with QUAL > 40
...
hist(log10(vcf.freebayes.HQ.Ae$Ae_tot.depth))
...
hist(vcf.freebayes.HQ.1.reform.Ae$MBR, xlim = c(0, 0.1), breaks = 50)
...
save(vcf.freebayes.HQ.1.reform.Ae, file = "~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/freebayes/vcf.freebayes.HQ.1.reform.Ae.Rdata")
...
dim(vcf.freebayes.HQ.Ol) # 1174345      25  
...
```{r}
load("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/freebayes/vcf.freebayes.HQ.2.test1.Ae.Rdata"); load("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/freebayes/vcf.freebayes.HQ.2.test1.Ol.Rdata")
...
vcf.freebayes.HQ.1.reform$gene_ID <- ANN_gene_ID
...
## check correlation between gene ID and allele ratio 
load("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/output/freebayes/vcf.freebayes.HQ.1.reform.Rdata")

# get MAF 
vcf.freebayes.HQ.1.reform$MAF_Ol <- round(pmin(vcf.freebayes.HQ.1.reform$Ol_alt.depth, vcf.freebayes.HQ.1.reform$Ae_ref.depth)/vcf.freebayes.HQ.1.reform$Ae_tot.depth, digits = 2) 
...
View(for_allele_specific_expression.Ae.het)
...
### calcualte genotype quality for freebayes 
unique(vcf.freebayes.HQ.1.reform$CIGAR) # 1X 
```
...
######## compare the two intersect dataset... 
tmp <- intersect(vcf.Ae.Ol.intersect.2, vcf.Ae.Ol.intersect)
length(tmp) # 59343 # common between combined & sepereate freebayes call 
...
# ####### compare SNPs between Da-Ae & Da-Ol-1 
