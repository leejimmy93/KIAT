...
# SNP calling   
```{r}
# using scripts /Network/Servers/avalanche.plb.ucdavis.edu/Volumes/Mammoth/Users/ruijuanli/SNP_parent/analysis/freebayes/freebayes_SNP_calling.sh before add read group & freebayes 
``` 

# using freebayes to call SNP (This result looks like is not from uniquely mapped data...)
```{r}
# call as diploid 
...
vcf.GATK.A01.filtered <- read.table("~/Desktop/Brassica_project/KIAT_RNA_seq/parent_SNP/data/GATK/as_diploid/filtered/xaa.intervals.vcf.filtered.vcf",as.is=T,na.strings = ".") 
vcf.GATK.filtered.reform <- SNP.GATK.reformat(vcf.GATK.A01.filtered, vcf.header.GATK)
dim(vcf.GATK.filtered.reform) # 68060 21       

#### filter (based on QUAL score and snpcluster)
# basic filtering based on snpcluster & QUAL 
vcf.GATK.HQ <- SNP.GATK.basic.filter(vcf.GATK.filtered.reform)
dim(vcf.GATK.HQ) # 29962 21 
``` 

# figure out why many 0/1 
```{r} 
head(vcf.GATK.reform.HQ) 

# reformat to get seperate read depth for Ae & Ol 
Ae.gt <- matrix(
  unlist(strsplit(vcf.GATK.reform.HQ$Ae_ref.alt.depth,split = ",")),
  nrow=nrow(vcf.GATK.reform.HQ),  
  byrow=TRUE
  ) 
...
Ol.gt <- matrix(
  unlist(strsplit(vcf.GATK.reform.HQ$Ol_ref.alt.depth,split = ",")),
  nrow=nrow(vcf.GATK.reform.HQ),  
  byrow=TRUE
  )
...
vcf.GATK.reform.HQ <- cbind(vcf.GATK.reform.HQ, Ae.gt, Ol.gt, stringsAsFactors=FALSE)

vcf.GATK.reform.HQ$Ae_alt.depth <- as.numeric(vcf.GATK.reform.HQ$Ae_alt.depth)
vcf.GATK.reform.HQ$Ae_ref.depth <- as.numeric(vcf.GATK.reform.HQ$Ae_ref.depth)
vcf.GATK.reform.HQ$Ol_alt.depth <- as.numeric(vcf.GATK.reform.HQ$Ol_alt.depth)
vcf.GATK.reform.HQ$Ol_ref.depth <- as.numeric(vcf.GATK.reform.HQ$Ol_ref.depth)


head(vcf.GATK.reform.HQ)

vcf.GATK.reform.HQ.filtered <- vcf.GATK.reform.HQ[which(vcf.GATK.reform.HQ$Ae_approx.depth > 3 & vcf.GATK.reform.HQ$Ae_approx.depth < 501 & vcf.GATK.reform.HQ$Ol_approx.depth > 3 & vcf.GATK.reform.HQ$Ol_approx.depth < 501),] 

dim(vcf.GATK.reform.HQ.filtered) # 40272    25  
...
# 1) check the ref/alt allele ratio for the het calls, distribution plot for Ae & Ol 
# get het & homo call for Ae & Ol 
vcf.GATK.reform.HQ.filtered.Ae.het <- vcf.GATK.reform.HQ.filtered[(vcf.GATK.reform.HQ.filtered$Ae_gt=="0/1"),]
# vcf.Ae.Ol.no.bolting.unique.HQ.filtered.Ae.ref <- vcf.Ae.Ol.no.bolting.unique.HQ.filtered[(vcf.Ae.Ol.no.bolting.unique.HQ.filtered$Ae_gt=="0/0"),]
# vcf.Ae.Ol.no.bolting.unique.HQ.filtered.Ae.alt <- vcf.Ae.Ol.no.bolting.unique.HQ.filtered[(vcf.Ae.Ol.no.bolting.unique.HQ.filtered$Ae_gt=="1/1"),]

table(vcf.GATK.reform.HQ.filtered$Ae_gt) 
...
pl <- ggplot(data = vcf.GATK.reform.HQ.filtered.Ae.het)
pl <- pl + geom_histogram(aes(Ae_alt.depth/Ae_ref.depth)) + scale_x_log10()
pl <- pl +labs(list(x="log10(alt.depth/ref.depth)"))
pl 

pl.allele.ratio.Ae(vcf.GATK.reform.HQ.filtered.Ae.het) 
