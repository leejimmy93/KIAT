...
F2_geno_data_with_missing_rate_filtered <- filter(F2_geno_data_with_missing_rate, missing_rate < 0.15)
...
# calculate two point rf & assign to linkage groups 
```{r}
...
# # assign marker to linkage groups ############ using the onemap method cannot assign them to individual linkage groups ########## 
...
#################### assign marker to specific linkage groups based on their chromosome assignement
...
load("~/Desktop/Brassica_project/KIAT_RNA_seq/F2/output/LG1.f2.ord.A01.Rdata")
LG1.f2.ord.A01
...
LG1.f2.safe.A01 <- make.seq(LG1.f2.ord.A01, "safe")
LG1.f2.safe.A01
# all order (all markers)
LG1.f2.final.A01 <- make.seq(LG1.f2.ord.A01, "force")
LG1.f2.final.A01

# print the recombination fraction mix 
rf.graph.table(LG1.f2.safe.A01) 
par(mfrow=c(1,1))
draw.map(LG1.f2.safe.A01, names= F, grid=F, cex.mrk=0.5, cex.grp=0.75, horizontal=T)

# map estimation for arbitrary order 
LG1.f2.arbi.A01 <- make.seq(twopts.f2.04.24, order[[1]])
LG1.f2.arbi.A01.map <- map(LG1.f2.arbi.A01)
rf.graph.table(LG1.f2.arbi.A01.map) 

draw.map(LG1.f2.arbi.A01.map, names= F, grid=F, cex.mrk=0.5, cex.grp=0.75, horizontal=T)  
...
###
load("~/Desktop/Brassica_project/KIAT_RNA_seq/F2/output/LG1.f2.ord.A02.Rdata")
LG1.f2.ord.A02
...
# safe mode order (only marker with unique positions)
LG1.f2.safe.A02 <- make.seq(LG1.f2.ord.A02, "safe")
LG1.f2.safe.A02
# all order (all markers)
LG1.f2.final.A02 <- make.seq(LG1.f2.ord.A02, "force")
LG1.f2.final.A02

# print the recombination fraction mix 
rf.graph.table(LG1.f2.final.A02) 
par(mfrow=c(1,1))
draw.map(LG1.f2.safe.A02, names= F, grid=F, cex.mrk=0.5, cex.grp=0.75, horizontal=T)

# map estimation for arbitrary order 
LG1.f2.arbi.A02 <- make.seq(twopts.f2.04.24, order[[1]])
LG1.f2.arbi.A02.map <- map(LG1.f2.arbi.A02)
rf.graph.table(LG1.f2.arbi.A02.map) 

draw.map(LG1.f2.arbi.A02.map, names= F, grid=F, cex.mrk=0.5, cex.grp=0.75, horizontal=T)   
```
...
#     No. individuals:     166 
#     No. markers:         5765 
#     Percent genotyped:   69 # the simulated data in onemap is 85% genotyped, maybe I should drop those with low percent? 
# 
...
# order marker 
LG1.f2.C01 <- make.seq(LG1.f2.ord.C01)
LG1.f2.C01
