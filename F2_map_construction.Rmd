...
# figure out the index positon within each chromosome 
F2_geno_data_2$Pos <- gsub("([[:print:]]+)(_)([[:digit:]]+)", "\\3", rownames(F2_geno_data_2))
sum(F2_geno_data_2$Chr=="chrC01")

F2_geno_data_2$index <- seq(1: nrow(F2_geno_data_2))
F2_geno_data_info <- F2_geno_data_2[,c("Chr", "Pos", "index")]

C01 <- F2_geno_data_info[F2_geno_data_info$Chr=="chrC01",]
C01$Pos <- as.numeric(C01$Pos)

C01[order(C01$Pos),]$index

sorted.data <- data[order(data$V1,data$V12,decreasing = TRUE),]
```
...
# LGs.f2 <- group(mark.all.f2, LOD = 6, max.rf = 0.25)
# LGs.f2
>>>>>>> cae03dbfa6c30a1cae4ecafd1b6490e387fc21c2
...
# use touchdown to order marker 
LG1.f2.ord <- order.seq(input.seq = mark.f2.A01, n.init = 5,
                        subset.search = "twopt",
                        twopt.alg = "rcd", THRES = 6,
                        draw.try = T, wait = 1,
                        touchdown = T)
>>>>>>> d29c2a4f639cffd5dce8579557b1ac3bb182f391
...
rf.graph.table(LG1.f2.final.C01) 
par(mfrow=c(1,1))
draw.map(LG1.f2.final.C01, names= F, grid=F, cex.mrk=0.5)

# map estimation for arbitrary order 
LG1.f2.arbi.C01 <- make.seq(twopts.f2.04.24, c(3843, 3922, 3923, 3970, 3804, 3841, 3851, 3852, 3853, 3855, 3857, 3858, 3860, 3856))
